...
function ZhBody() {
  return (
    <>
      <p>
        对<em>一个</em>变量在不确定下进行推理，是
        <Link href="/knowledge/bayesian-statistics">贝叶斯</Link>的 故事。但真实的问题涉及
        <em>许多</em>相互关联的不确定变量——症状与疾病、原因与结果、信号与状态——而对
        所有这些变量的完整<Term>联合分布</Term>，大得像天文数字。<Term>概率图模型</Term>
        （PGM）用一个漂亮的 洞见让这变得可处理：<strong>把变量之间的依赖关系画成一张图</strong>
        ，而那张图既把分布压缩成可管理的
      ...
      <p>
        ...
        <Link href="/knowledge/causal-discovery">因果发现</Link>
        。这一页讲核心的想法（把独立性化为图）、
      ...
      <KSection id="why" eyebrow="01" title="驯服那个不可能的联合分布">
        <p>
          ...
          仅仅 30 个是/否变量，联合分布就有超过<em>十亿</em>
          个条目。你存不下它，更别说从数据中估计它、或用它
          来计算。必须利用某种结构，而拯救我们的那个结构是<strong>独立性</strong>
      ...
      <KSection id="idea" eyebrow="02" title="把条件独立化为图">
        <p>
          核心的想法：<strong>一张图编码了哪些变量（在条件上）独立于哪些</strong>
          。每个变量是一个节点；两个 节点之间的一条边意味着一个直接的依赖；一条<em>缺失</em>
          的边断言一个
          <Link href="/knowledge/causal-inference">条件独立</Link>
          （「一旦我知道了它的父节点，这个变量就再也
      ...
      <KSection id="bayesnet" eyebrow="03" title="贝叶斯网络">
        <p>
          最常见的 PGM 是<Term>贝叶斯网络</Term>（或称信念网络）：一个<Term>有向无环图</Term>
          （DAG，与因果图 同样的结构），其中每个节点只依赖于它的<em>父节点</em>
          。每个节点带着一张小表——它<em>在给定父节点下</em>
        ...
        />
        <p>
          那个经典的玩具例子：<em>雨</em>和一个<em>洒水器</em>各自都能让<em>草地</em>
          变湿，而雨还影响洒水器
      ...
      <KSection id="factorise" eyebrow="04" title="因子分解：为什么它可处理">
        <p>
          数学上的回报是<Term>因子分解</Term>
          。一个贝叶斯网络说，完整的联合分布就是每个变量在给定其父节点下 概率的<em>乘积</em>：
        </p>
        <Formula label="The joint probability of all variables x-1 through x-n equals the product over i of the probability of x-i given its parents.">
          {String.raw`P(X_1, \dots, X_n) = \prod_{i=1}^{n} P\big(X_i \mid \text{parents}(X_i)\big)`}
        </Formula>
        <p>
          这就是一行里的全部魔法。你不是存一张覆盖所有变量的巨大的表，而是存许多<em>小</em>
      ...
      <KSection id="inference" eyebrow="05" title="用证据推理：推断">
        <p>
          ...
          <TeX>{String.raw`P(\text{query} \mid \text{evidence})`}</TeX>{" "}
          的问题：「在草地是湿的、并且天阴的
        ...
        <Callout type="pitfall">
          <p>
            陷阱在于，<strong>精确推断一般而言在计算上很难</strong>（对任意的图是 NP
            难的）。对于漂亮的结构 （树），通过<Term>信念传播</Term>
            它是高效的——在相邻节点之间传递「信念」消息，直到它们达成一致 （注意它与{" "}
            <Link href="/knowledge/graph-neural-networks">GNN 的消息传递</Link>的亲缘关系）。对于
            有环的、复杂的网络，你就退而求其次用<strong>近似</strong>推断——
            <Link href="/knowledge/computational-statistics">MCMC</Link>{" "}
      ...
      <KSection id="learning" eyebrow="06" title="学习模型">
        <p>
          一个 PGM 有两样东西要学：<em>参数</em>
          （概率表，在给定图的情况下——通常从数据中可以直接得到）和
          <em>结构</em>（图本身——哪些边存在）。从观测数据中学习结构，正是
          <Link href="/knowledge/causal-discovery">因果发现</Link>
          ，连同它所有的困难（你往往只能恢复出一个 等价类）。实践中结构常常来自
          <strong>领域知识</strong>——一个专家画出依赖图——而只学习参数，这是一个 真正的强项：PGM 让你
          <em>编码你所知道的</em>，并学习其余的。
        </p>
      </KSection>
      ...
      <KSection id="applied" eyebrow="07" title="它在我工作中的体现">
        <Callout type="applied" label="不确定下透明的推理">
          <p>
            PGM 在有<strong>许多相互关联的不确定变量</strong>、且你需要既有原则又<em>可解释</em>
            的推理之处 最重要。与黑箱模型不同，一个贝叶斯网络<strong>展示它的推理</strong>
            ——这张图是一份「什么依赖于
          ...
          <p>
            它还统一了好几页：它是带许多变量的
            <Link href="/knowledge/bayesian-statistics">贝叶斯</Link>更新， 它的 DAG 就是
            <Link href="/knowledge/causal-inference">因果推断</Link>的图，学习它的结构是
            <Link href="/knowledge/causal-discovery">因果发现</Link>，而它的推断呼应
            <Link href="/knowledge/graph-neural-networks">GNN 的消息传递</Link>
            。需要的纪律是尊重那个
            <strong>推断代价</strong>
      ...
      <KSection id="refresher" eyebrow="08" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              许多变量上的完整<strong>联合分布</strong>大得像天文数字——PGM 靠利用
              <strong>独立性</strong>来 驯服它。
            </li>
            <li>
              一张<strong>图编码条件独立</strong>：节点 = 变量，一条缺失的边 =
              一个独立性。一份紧凑、可读的 依赖地图。
            </li>
            <li>
              一个<strong>贝叶斯网络</strong>是一个 DAG，其中每个节点依赖于它的
              <strong>父节点</strong>；用 每个节点的小表，它指定了整个联合分布。
            </li>
            <li>
              <strong>因子分解</strong>{" "}
              <TeX>{String.raw`P(X_1..X_n)=\prod_i P(X_i\mid \text{parents})`}</TeX>{" "}
              把一张巨大的表变成许多 小表——可存储、可学习、可计算。
            </li>
            <li>
              <strong>推断</strong> = <TeX>{String.raw`P(\text{query}\mid\text{evidence})`}</TeX>
              。在树上容易（
              <strong>信念传播</strong>）；一般而言 <strong>NP 难</strong> → MCMC / 近似。
            </li>
            <li>
              学习<strong>参数</strong>（容易）+ <strong>结构</strong>（=
              因果发现，或来自领域知识）。强项：
