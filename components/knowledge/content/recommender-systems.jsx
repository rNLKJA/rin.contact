...
function MFFigure({ caption, ariaLabel, usersItemsLabel, kFactorsLabel, perUserItemLabel, ratingDotLabel }) {
  return (
    <Figure caption={caption}>
      ...
      >
        {/* R */}
        <rect x="20" y="35" width="80" height="80" rx="2" fill="none" stroke="currentColor" strokeWidth="1.3" />
        <text x="60" y="78" textAnchor="middle" fontSize="13" fontFamily="monospace" fill="currentColor">R</text>
        <text x="60" y="128" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.6">{usersItemsLabel}</text>
        <text x="118" y="80" textAnchor="middle" fontSize="13" fill="currentColor">≈</text>
        {/* U */}
        <rect x="140" y="35" width="26" height="80" rx="2" fill="none" stroke="#FF3C3C" strokeWidth="1.3" />
        <text x="153" y="80" textAnchor="middle" fontSize="11" fontFamily="monospace" fill="#FF3C3C">U</text>
        <text x="153" y="128" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.6">m × k</text>
        <text x="180" y="80" textAnchor="middle" fontSize="12" fill="currentColor">×</text>
        {/* V^T */}
        <rect x="198" y="58" width="80" height="26" rx="2" fill="none" stroke="#FF3C3C" strokeWidth="1.3" />
        <text x="238" y="76" textAnchor="middle" fontSize="11" fontFamily="monospace" fill="#FF3C3C">Vᵀ</text>
        <text x="238" y="100" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.6">k × n</text>
        {/* note */}
        <text x="360" y="66" textAnchor="middle" fontSize="8.5" fontFamily="monospace" fill="currentColor" opacity="0.7">{kFactorsLabel}</text>
        <text x="360" y="82" textAnchor="middle" fontSize="8.5" fontFamily="monospace" fill="currentColor" opacity="0.7">{perUserItemLabel}</text>
        <text x="360" y="98" textAnchor="middle" fontSize="8.5" fontFamily="monospace" fill="currentColor" opacity="0.7">{ratingDotLabel}</text>
      </svg>
    </Figure>
  );
}
...
function ZhBody() {
  return (
    <>
      <p>
        现代生活中出人意料的一大部分，是被<Term>推荐系统</Term>塑造的：一个流媒体服务推到你眼前的电影、
        一家商店建议的商品、一个信息流接下来给你看的帖子。它们是个性化的引擎，而它们核心的想法真正优雅——从
        每个人过去喜欢过什么的模式里，预测一个特定的人会有多喜欢一个特定的东西，并据此排序。
      </p>
      <p>
        它值得好好了解，有两个原因。它的机制漂亮地连回
        <Link href="/knowledge/pca-dimensionality-reduction">降维</Link>——核心的技术就是同一个潜在因子的
      ...
      <KSection id="problem" eyebrow="01" title="预测你会喜欢什么">
        <p>
          ...
          一个评分、一次点击、一次购买。问题在于这张表极度<strong>稀疏</strong>：任何一个人都只与所有物品里
          极小的一部分有过交互。推荐器的工作就是<em>把空白填上</em>——预测缺失的格子——然后推荐它预测你会打
          最高分的物品。
        </p>
        <p>
          做这件事有两种根本不同的方式，区别在于它们去哪里寻找信号：在<em>物品</em>本身，还是在其他用户
          构成的<em>人群</em>。
        </p>
      </KSection>
      ...
      <KSection id="content" eyebrow="02" title="基于内容的过滤：更多你喜欢过的那类">
        <p>
          <Term>基于内容的过滤</Term>看的是<em>物品</em>。它从你过去喜欢过的东西的特征里，建立起一个「你
          喜欢什么」的画像，然后推荐有相似特征的物品。喜欢过好几部硬科幻电影？这里又有一部标着科幻的。物品
          特征可以是显式的（类型、作者、价格），也可以是描述的学习得到的
          <Link href="/knowledge/natural-language-processing">文本嵌入</Link>，而「相似」就是 NLP 页里那个
          同样的向量距离的想法。
        </p>
        <p>
      ...
      >
        <p>
          <Term>协同过滤</Term>完全忽略物品特征，只用交互的模式——群体的智慧。直觉是：「过去与你意见一致的
          人，未来也会与你一致。」两种风味：
        </p>
        <ul>
          <li>
          ...
          <li>
            <Term>物品—物品</Term>——找到倾向于被同一批人喜欢的物品，把（在那种共同喜欢的意义上）与你打过
            高分的物品相似的物品推荐给你。（「购买此商品的顾客也购买了……」）这是大多数大规模系统的驱动力，
            因为物品—物品的关系比用户口味更稳定。
          </li>
        </ul>
        <p>
          协同过滤的魔力在于，它<strong>不需要知道物品实际上是什么</strong>——只需要知道谁与什么交互过。
      ...
      >
        <p>
          驱动了现代协同过滤——并著名地赢得了 Netflix 大奖——的那个突破是<Term>矩阵分解</Term>。想法是：把
          巨大稀疏的用户—物品评分矩阵 <TeX>{String.raw`R`}</TeX> 近似为两个小得多的稠密矩阵的乘积：
        </p>
        <Formula label="The rating matrix R, of size users by items, is approximately equal to U times V transpose, where U is users by k and V is items by k.">
          {String.raw`R_{\,m \times n} \;\approx\; U_{\,m \times k}\, V_{\,n \times k}^{\top}`}
        </Formula>
        <p>
          每个用户变成一个由 <TeX>{String.raw`k`}</TeX> 个<Term>潜在因子</Term>组成的短向量，每个物品也是。
        ...
        <p>
          ...
          <strong>低秩、潜在维度</strong>的想法——把一个巨大的矩阵压缩进几个有意义的维度——这正是为什么线性
          代数的基础在这里直接见效。它还优雅地处理稀疏性：你只在你<em>确实</em>观测到的格子上拟合，而分解会
          泛化到其余的格子。
        </p>
      </KSection>
      ...
      <KSection id="coldstart" eyebrow="05" title="冷启动问题">
        <p>
          协同过滤的一大弱点是<Term>冷启动问题</Term>：它需要交互历史，而一个全新的用户或物品一点历史都没有。
          你没法给一个你一无所知的人推荐，你也没法把一个刚加进来、还没人碰过的物品推上去。这是每一个新推荐器
          核心处那个先有鸡还是先有蛋的难题。
        </p>
        <p>
          标准的修法是走<Term>混合</Term>路线：对新用户和新物品依靠<em>基于内容</em>的方法（它只需要特征），
          然后随着交互历史的积累转向<em>协同</em>过滤。大多数生产系统正是因为这个原因而是混合的——每种方法都
          覆盖另一种的盲点。
        </p>
      </KSection>
      ...
      <KSection id="evaluation" eyebrow="06" title="衡量成功：关键在于排序">
        <p>
          评估推荐器比一个单一的准确率数字更微妙，因为要紧的是你展示之物的<em>顺序</em>，而非一个精确的
          评分。指标是<Link href="/knowledge/model-evaluation">以排序为中心的</Link>：<Term>precision@k</Term>
          （在前 k 个推荐里，有多少是相关的？）和 <Term>NDCG</Term>（它还奖励把最好的物品放到最高）。你关心的
          是列表的顶部——没人会滚动到第 200 条推荐。
        </p>
        <Callout type="pitfall">
          <p>
            更深的陷阱是<strong>离线—在线鸿沟</strong>。一个在历史数据上分数漂亮的推荐器，可能在真实用户那里
            一败涂地，因为离线数据只记录了人们对<em>旧</em>系统的推荐做了什么——它没法告诉你他们会对真正新的
      ...
      <KSection id="issues" eyebrow="07" title="气泡与反馈回路">
        ...
        <ul>
          <li>
            <Term>流行度偏差</Term>——群体的最爱被推荐得最多，于是它们被交互得最多，于是它们被推荐得更多。
            富者愈富，而小众的物品始终不可见。
          </li>
          <li>
            <Term>过滤气泡 / 回音室</Term>——通过给你看越来越多你已经参与的东西，系统收窄了你的世界，这对
            电影是无害的，对新闻和观点却是腐蚀性的。
          </li>
          <li>
            <Term>反馈回路</Term>——模型的推荐成为它接下来训练所用的数据，于是它从自己的影响里学习，并可能
            失控地盘旋上升。这是<Link href="/knowledge/reinforcement-learning">奖励塑形</Link>问题换了个样子：
      ...
      <KSection id="applied" eyebrow="08" title="它在我工作中的体现">
        <Callout type="applied" label="排序、潜在因子与反馈回路">
          <p>
            推荐系统不是政府分析师的核心工具，但它的机制可以推广到任何<strong>排序或优先级排定</strong>问题——
            从一长串里把最值得关注的案件、物品或线索浮现出来，这是一种不断重现的形状。<strong>矩阵分解 /
            潜在因子</strong>的想法就是与 <Link href="/knowledge/pca-dimensionality-reduction">PCA</Link> 相同的
            低秩压缩，而那些<strong>排序指标</strong>（precision@k）是你评判任何「给我看前 N 个」系统的方式。
          </p>
          <p>
            不过，最能迁移过来的，是那个警示性的一半。<strong>反馈回路</strong>——一个在自己过去输出的后果
            之上训练的模型——是一个远超推荐器的陷阱：任何作用于世界、然后又从那个被改变的世界里学习的系统，
            都有把自己的偏差固化下来的风险，这直接连到<Link href="/knowledge/fairness-bias">公平</Link>和
            <Link href="/knowledge/mlops-monitoring">漂移</Link>问题。知道这个失败模式，正是让你能去提防它的
            东西。
          </p>
        </Callout>
      </KSection>
        ...
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              ...
              <strong>协同</strong>（用群体的交互——「像你这样的人喜欢」；物品—物品驱动大多数大系统）。
            </li>
            <li>
              <strong>矩阵分解</strong>（<TeX>{String.raw`R \approx U V^\top`}</TeX>）为每个用户/物品学出短的
              <strong>潜在因子</strong>向量；评分 = 点积。与 <strong>PCA</strong> 同一个低秩的想法。
            </li>
            <li>
              <strong>冷启动问题</strong>（新用户/物品，无历史）→ 走<strong>混合</strong>（新的用内容，历史
              增长后用协同）。
            </li>
            <li>
              用<strong>排序</strong>评估（precision@k、NDCG）——但留意<strong>离线—在线鸿沟</strong>；由实时
              A/B 测试评判。
            </li>
            <li>
              重大的问题：<strong>流行度偏差、过滤气泡、反馈回路</strong>——推荐器塑造行为，而不只是预测它。
            </li>
          </ul>
        </Callout>
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#6E6E6E] mt-6 [text-wrap:pretty]">
