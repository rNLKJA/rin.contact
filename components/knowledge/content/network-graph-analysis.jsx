...
const LEFT = [[55, 50], [40, 95], [90, 40], [80, 100], [110, 75]];
const RIGHT = [[385, 50], [400, 95], [350, 40], [360, 100], [330, 75]];
const LEFT_EDGES = [[55, 50, 90, 40], [55, 50, 40, 95], [40, 95, 80, 100], [90, 40, 110, 75], [80, 100, 110, 75]];
const RIGHT_EDGES = [[385, 50, 350, 40], [385, 50, 400, 95], [400, 95, 360, 100], [350, 40, 330, 75], [360, 100, 330, 75]];
    ...
    <Figure caption={caption}>
      ...
      >
        {LEFT_EDGES.concat(RIGHT_EDGES).map(([x1, y1, x2, y2], i) => (
          <line key={`e${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="currentColor" strokeWidth="1" opacity="0.5" />
        ))}
        {LEFT.concat(RIGHT).map(([x, y], i) => (
          <circle key={`n${i}`} cx={x} cy={y} r="9" fill="none" stroke="currentColor" strokeWidth="1.4" />
        ))}
        ...
        <text x="220" y="118" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="#FF3C3C">{brokerLabel}</text>
      </svg>
    </Figure>
  );
}
  ...
  return (
    <>
      <p>
        ...
        <strong>connections between them</strong>. Who calls whom, which accounts move money to which,
        which people appear together. <Term>Network analysis</Term> takes those relationships as the
        primary object of study, and a surprising amount of insight that's invisible row-by-row becomes
        obvious once you look at the structure.
      </p>
      <p>
        It's a discipline I lean on directly in intelligence work, where the question is often "who is
        the key connector here, and what's the hidden group?" — a question you literally cannot answer
        without modelling the links. This page is the practical toolkit: how to represent a network,
        the handful of measures that find the important nodes, how to find communities, and the traps
        that make network analysis lie.
      </p>
      ...
      <KSection id="why" eyebrow="01" title="Relationships as data">
        <p>
          ...
          between them). That's it — and yet representing data this way unlocks questions that tabular
          data can't express: <em>how far apart</em> are two people through intermediaries? <em>Who</em>{" "}
          sits at the centre? <em>Which</em> tightly-knit group does someone belong to?
        </p>
        <p>
          The shift in mindset is the whole point: stop asking "what are this node's attributes?" and
          start asking "what is this node's <em>position</em> in the structure?" Two people with
      ...
      <KSection id="anatomy" eyebrow="02" title="Nodes, edges & structure">
        ...
        <ul>
          <li>
            <Term>Directed vs undirected</Term> — "called" has a direction (A → B); "appeared together"
            doesn't. Direction matters for who influences whom.
          </li>
          <li>
          ...
          <li>
            <Term>Paths &amp; connectivity</Term> — a <em>path</em> is a route along edges between two
            nodes; the <em>shortest path</em> (degrees of separation) underpins several of the measures
            below. A network can also break into disconnected <em>components</em>.
          </li>
        </ul>
        <p>
      ...
      <KSection id="degree" eyebrow="03" title="Who's busy: degree centrality">
        <p>
          The simplest measure is <Term>degree centrality</Term> — just count a node's connections. The
          degree <TeX>{TEX.ki}</TeX> of node <TeX>{TEX.i}</TeX> is the number of edges touching it. High
          degree means a hub: someone connected to many others.
        </p>
        <p>
          It's a fine first pass — and often misleading on its own. A node can have a hundred
          connections but sit on the edge of the network, while a node with three connections sits at
          its only bridge. Degree counts <em>quantity</em>, not <em>position</em>. The next two measures
          fix that.
        </p>
      </KSection>
      ...
      <KSection id="betweenness" eyebrow="04" title="Who's the broker: betweenness centrality">
        ...
        <p>
          ...
          between parts of the network. These are often the most consequential nodes of all — remove one
          and the network can fracture — even when their raw degree is modest.
        </p>
        <BrokerFigure
      ...
      <KSection id="eigenvector" eyebrow="05" title="Who's influential: eigenvector & PageRank">
        <p>
          <Term>Eigenvector centrality</Term> captures a subtler idea: it's not just <em>how many</em>{" "}
          connections you have, but <em>how important</em> they are. You're influential if you're
          connected to influential people. That's circular by design, and the maths resolves the
          circularity elegantly — a node's score is proportional to the sum of its neighbours' scores:
        </p>
        ...
        <p>
          ...
          <Link href="/knowledge/linear-algebra">eigenvector</Link> of the network's adjacency matrix
          (hence the name, and a neat callback to the linear-algebra page). <Term>PageRank</Term> — the
          algorithm that originally ranked the web — is a famous variant: a page is important if
          important pages link to it. The same logic finds the quietly powerful node that brokers don't
          capture: not the busiest, not the bridge, but the one embedded among the influential.
        </p>
        <Callout type="intuition">
          <p>
            ...
            <strong>"who is well-connected to the well-connected?"</strong>. The same node rarely tops
            all three, and the differences are themselves informative.
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="community" eyebrow="06" title="Finding the groups: community detection">
        <p>
          Beyond individual nodes, networks have <Term>community structure</Term> — clusters of nodes
          more densely connected to each other than to the rest. Finding them reveals the cells,
          factions, or interest groups inside a network, and it's the graph cousin of{" "}
          <Link href="/knowledge/clustering">clustering</Link>.
        </p>
        <p>
          The standard objective is <Term>modularity</Term>: a partition of the network scores high when
          there are many edges <em>within</em> groups and few <em>between</em> them, compared to what
          you'd expect by chance. Algorithms like Louvain and Leiden optimise it efficiently, and
          label-propagation methods offer a fast alternative. The output — "these twenty nodes form a
          tight group barely connected to the rest" — is often the single most operationally useful
          thing network analysis produces.
        </p>
      </KSection>
      ...
      <KSection id="pitfalls" eyebrow="07" title="Where it misleads">
        ...
        <ul>
          <li>
            <Term>Centrality isn't importance.</Term> A high score is a structural fact, not a verdict.
            The most central node might be a switchboard operator, not a kingpin. Centrality{" "}
            <em>nominates</em> nodes for attention; it doesn't convict them.
          </li>
          <li>
            <Term>Missing-edge bias.</Term> Your network is only the connections you happened to record.
            Missing edges (an unobserved relationship) can completely change who looks central, and
            absence of an edge is rarely evidence of absence of a link.
          </li>
          <li>
            <Term>The hairball.</Term> A big network drawn as a tangle of crossing lines looks
            impressive and shows nothing. Lean on the <em>measures</em> (centrality tables, community
            labels), not the pretty-but-unreadable picture.
          </li>
          <li>
            <Term>Spurious nodes.</Term> A shared taxi rank or a customer-service line can make
            unrelated people look connected. Garbage edges produce confident-looking, wrong structure.
          </li>
        </ul>
      </KSection>
      ...
      <KSection id="applied" eyebrow="08" title="Where it shows up in my work">
        <Callout type="applied" label="Finding the connector & the hidden group">
          <p>
            ...
            transactions, model them as a graph, and ask <strong>who is the key connector</strong> and{" "}
            <strong>what is the hidden community</strong>. That's exactly degree vs{" "}
            ...
            <strong>community detection</strong> to surface the cell that isn't obvious case-by-case.
          </p>
          <p>
            ...
            <strong>missing edge</strong> can move the whole picture, and refusing to be impressed by a
            hairball. It connects to the rest of the section too — communities are{" "}
            <Link href="/knowledge/clustering">clustering</Link> on a graph, eigenvector centrality is{" "}
            <Link href="/knowledge/linear-algebra">linear algebra</Link>, and the leads it produces feed
            straight into <Link href="/knowledge/intelligence-analysis">structured analysis</Link>.
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="09" title="Refresh in 60 seconds">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              ...
              <strong>betweenness</strong> (who's the broker/bridge — <TeX>{TEX.betweennessShort}</TeX>),{" "}
              <strong>eigenvector/PageRank</strong> (who's connected to the well-connected — an{" "}
              <TeX>{TEX.AxShort}</TeX> eigenvector).
            </li>
            <li>
              <strong>Community detection</strong> (modularity, Louvain/Leiden) finds densely-connected
              groups — clustering on a graph; often the most useful output.
            </li>
            <li>
              Report the centralities <strong>together</strong> — the same node rarely tops all three,
              and the differences are informative.
            </li>
            <li>
        ...
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#6E6E6E] mt-6 [text-wrap:pretty]">
          Centrality definitions, modern community-detection methods, and the centrality-isn't-importance
          caution reflect current network-analysis references alongside hands-on link analysis.
        </p>
      </KSection>
    </>
  );
    ...
    <>
      ...
      <KSection id="why" eyebrow="01" title="关系即数据">
        <p>
          ...
          的问题：两个人通过中间人相距<em>多远</em>？<em>谁</em>坐在中心？某人属于<em>哪个</em>紧密的
          群组？
        </p>
        <p>
      ...
      <KSection id="anatomy" eyebrow="02" title="节点、边与结构">
        ...
        <ul>
          ...
          <li>
            <Term>路径与连通性</Term>——一条<em>路径</em>是沿着边在两个节点间的一条路线；<em>最短
            路径</em>（分隔度）支撑着下面几个度量。一个网络也可能断裂成互不相连的<em>分量</em>。
          </li>
        </ul>
        <p>
      ...
      <KSection id="betweenness" eyebrow="04" title="谁是中间人：介数中心性">
        ...
        <p>
          其中 <TeX>{TEX.sigmaSt}</TeX> 是从 <TeX>{TEX.s}</TeX> 到 <TeX>{TEX.t}</TeX> 的最短路径数，而{" "}
          <TeX>{TEX.sigmaStv}</TeX> 是其中经过 <TeX>{TEX.v}</TeX> 的数量。一个高介数的节点是一个
      ...
      <KSection id="eigenvector" eyebrow="05" title="谁有影响力：特征向量与 PageRank">
        ...
        <Callout type="intuition">
          <p>
            这三个度量回答三个不同的问题，而一个好的分析师会把它们一起报告：度 = <strong>「谁忙？」
            </strong>、介数 = <strong>「谁是守门人？」</strong>、特征向量/PageRank = <strong>「谁与
            人脉广的人脉广？」</strong>。同一个节点很少在三者上都居首，而它们之间的差异本身就有
            信息量。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="community" eyebrow="06" title="找出群组：社区发现">
        <p>
          ...
          它们，便揭示了一个网络内部的小团体、派系或利益集团，而它是<Link href="/knowledge/clustering">聚类
          </Link>在图上的表亲。
        </p>
        <p>
      ...
      <KSection id="refresher" eyebrow="09" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              一个图是<strong>节点 + 边</strong>。转变在于：问一个节点在结构中的<strong>位置</strong>，
              而不只是它的属性。
            </li>
            <li>
              三个中心性，三个问题：<strong>度</strong>（谁忙）、<strong>介数</strong>（谁是中间人/桥
              ——<TeX>{TEX.betweennessShort}</TeX>）、<strong>特征向量/PageRank</strong>（谁与人脉广的
              相连——一个 <TeX>{TEX.AxShort}</TeX> 特征向量）。
            </li>
            <li>
            ...
            <li>
              陷阱：<strong>中心性 ≠ 重要性</strong>（一条线索，而非一个裁决）、<strong>缺边偏倚
              </strong>、读不懂的<strong>毛球图</strong>，以及<strong>伪边</strong>。
            </li>
          </ul>
        </Callout>
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#6E6E6E] mt-6 [text-wrap:pretty]">
