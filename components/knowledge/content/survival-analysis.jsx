...
function CensoringFigure({ caption, ariaLabel, studyEndsLabel, legendLabel }) {
  return (
    <Figure caption={caption}>
      ...
      >
        <line x1="360" y1="14" x2="360" y2="136" stroke="#FF3C3C" strokeWidth="1" strokeDasharray="3 3" opacity="0.6" />
        <text x="360" y="146" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="#FF3C3C">{studyEndsLabel}</text>
        {SUBJECTS.map(([end, event], i) => {
          const y = 22 + i * 24;
          return (
            <g key={i}>
              <line x1="30" y1={y} x2={end} y2={y} stroke="currentColor" strokeWidth="1.4" opacity="0.7" />
              {event ? (
        ...
        <text x="30" y="146" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.6">{legendLabel}</text>
      </svg>
    </Figure>
  );
}
...
function ZhBody() {
  return (
    <>
      <p>
        许多最重要的问题不是「这会不会发生？」而是「<em>还要多久</em>它才发生？」——还要多久一个病人复发、
        一个客户流失、一台机器故障、一个案件被解决、一个人再犯。<Term>生存分析</Term>（或称事件时间分析）正是
      ...
      <KSection id="why" eyebrow="01" title="还要多久……？">
        <p>
          ...
          的一次<Link href="/knowledge/linear-statistical-models">回归</Link>？你不能——对许多对象来说事件
          <em>还没发生</em>，所以他们的时间是未知的。把它当作<Link href="/knowledge/statistical-modelling">
          分类</Link>（「事件在时刻 T 之前发生了吗？」）？你扔掉了<em>何时</em>这一丰富的信息，而答案任意地
          取决于你把 T 画在哪里。
        </p>
        <p>
      ...
      <KSection id="censoring" eyebrow="02" title="删失：定义这个领域的想法">
        <p>
          <Term>删失</Term>是生存分析的核心。一个对象是<Term>右删失</Term>的，是当你知道他们<em>存活到</em>
      ...
      <KSection id="functions" eyebrow="03" title="描述生存的两种方式">
        <p>生存由两个互补的函数来描述。<Term>生存函数</Term>：</p>
        <Formula label="S of t is the probability that the survival time capital T is greater than t.">
          {String.raw`S(t) = \Pr(T > t)`}
        </Formula>
        <p>
          ——存活（没有发生事件）超过时刻 <TeX>{String.raw`t`}</TeX> 的概率。它从 1 开始，一级一级往 0 下降。
          <Term>风险函数</Term> <TeX>{String.raw`h(t)`}</TeX> 取了一个不同的角度：它是在时刻{" "}
          <TeX>{String.raw`t`}</TeX> 事件的<em>瞬时</em>率，<em>在</em>你已经存活到那么远的条件下——对那些
      ...
      <KSection id="km" eyebrow="04" title="Kaplan-Meier：估计曲线">
        <p>
          <Term>Kaplan-Meier 估计量</Term>是从数据中估计 <TeX>{String.raw`S(t)`}</TeX> 的主力，而它优雅地
          处理删失。它产出那条熟悉的<strong>阶梯曲线</strong>：生存保持平直，然后在每一次事件实际发生时下降
      ...
      <KSection id="logrank" eyebrow="05" title="比较各组：对数秩检验">
        <p>
          ...
          一条 Kaplan-Meier 曲线，用<Term>对数秩检验</Term>来比较它们——一个关于两条（或更多）生存曲线之间的
          差异是否大过偶然所能解释的<Link href="/knowledge/statistics">假设检验</Link>。它是生存分析里比较组
          均值的对应物，为尊重删失而建。它告诉你曲线是否<em>不同</em>，却不告诉你差多少、或在调整其他因素的
          同时——而这正是 Cox 模型登场的地方。
        </p>
      </KSection>
      ...
      <KSection id="cox" eyebrow="06" title="Cox 比例风险模型">
        <p>
          ...
          <Term>Cox 比例风险模型</Term>是占主导地位的那个。它不直接建模生存曲线，而是建模<em>风险</em>，因为
          风险更稳定、更易处理。它的形式：
        </p>
        <Formula label="The hazard for an individual with covariates x at time t equals a baseline hazard h-zero of t, times the exponential of beta-1 x-1 plus dot dot dot plus beta-p x-p.">
          {String.raw`h(t \mid \mathbf{x}) = h_0(t)\, \exp(\beta_1 x_1 + \cdots + \beta_p x_p)`}
        ...
        <p>
          美妙之处在于它是<Term>半参数</Term>的：基线风险 <TeX>{String.raw`h_0(t)`}</TeX>——风险总体上如何随
          时间变化——被留作<em>未指定</em>，所以你对生存曲线的形状不作任何假设。你只估计{" "}
          <TeX>{String.raw`\beta`}</TeX> 系数，即每个协变量的效应。对一个系数取指数，得到一个
          <Term>风险比</Term>：<TeX>{String.raw`e^{\beta} = 2`}</TeX> 意味着那个因素在任何时刻把瞬时风险
          <em>加倍</em>；低于 1 则是保护性的。那个单一、可解释的数字——「这个因素把风险乘以 X」——正是为什么
      ...
      <KSection id="assumption" eyebrow="07" title="比例风险假设">
        <p>
          Cox 模型用一个关键的假设买来它的灵活性，这个假设藏在名字里：<Term>比例风险</Term>。它假设一个
          协变量的效应是对风险在<em>所有</em>时刻的一个<em>恒定的乘数</em>——两组之间的风险比不随时间流逝而
          改变。
        </p>
        <Callout type="pitfall">
          <p>
            ...
            时间变化的故事平均掉了。所以<strong>务必检查它</strong>（用残差图或一个正式的检验），在它失败时
      ...
      <KSection id="applied" eyebrow="08" title="它在我工作中的体现">
        <Callout type="applied" label="当问题是「还要多久？」时">
          <p>
            ...
            一个人在一个项目里待多久才退出。生存分析给我的最有价值的东西，是围绕<strong>删失</strong>的
            纪律——认识到那些<em>还没</em>完成的案例携带真实的信息，而把它们扔掉（那个诱人的捷径）会系统性地
            把答案偏向任何很快就完成的东西。
          </p>
          <p>
            <strong>Kaplan-Meier</strong> 是展示「随时间有多大比例仍然留存」的诚实方式，<strong>对数秩检验
            </strong>恰当地比较两组的时间线，而 <strong>Cox 模型</strong>在调整混杂因素的同时，给出一个可解释
            的<em>风险比</em>——「这个因素把风险乘以 X」——这与<Link href="/knowledge/causal-inference">因果
            推断</Link>的心态天然相配。懂得<strong>比例风险</strong>假设，正是让那个风险比保持诚实、而非一个
            变化中故事的便利平均的东西。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="09" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              生存分析回答<strong>「还要多久事件才发生？」</strong>——普通回归/分类会失败，因为事件还没发生在
              每个人身上。
            </li>
            <li>
              <strong>删失</strong>是关键的想法：一个右删失的对象在某个时刻之前无事件（一个真实的下界）。别把
              它们扔掉，也别把删失当作事件——两者都让结果有偏。
            </li>
            <li>
              两个视角：<strong>生存</strong> <TeX>{String.raw`S(t)=\Pr(T>t)`}</TeX>（撑过 t 的比例）和
            ...
            <li>
              检查<strong>比例风险假设</strong>——一个随时间恒定的风险比；当它被违反时（曲线交叉），那个单一的
              数字会误导人。
            </li>
          </ul>
        </Callout>
