  ...
  return (
    <>
      <p>
        ...
        对，它真的能，达到一种你能量化的精度。那种近乎魔法的本事，是<Term>抽样理论</Term>
        的成就：那套
      ...
      <KSection id="why" eyebrow="01" title="为什么一个样本能代表所有人">
        <p>
          前提是，你很少需要测量每一个人（一次<Term>普查</Term>
          ）才能了解每一个人。一个挑得好的子集， 以一小部分的成本与时间，承载着整体的信息。关键词是
          <strong>挑得好</strong>——而让它运作的引擎， 是从
          <Link href="/knowledge/probability">概率</Link>页借来的一个单一想法：
          <strong>随机性</strong>。
        </p>
        <p>
          如果总体里的每一个成员都有一个已知的、非零的被选中机会，那么概率的法则就让你能从样本泛化到
          总体，<em>并</em>
          给这个泛化加上诚实的误差棒。把随机性拿掉，那座桥就塌了——你手里只有一堆来自
          碰巧应答者的回答，谁也不特别代表。
        </p>
      </KSection>
      ...
      <KSection id="probability" eyebrow="03" title="概率抽样：能泛化的方法">
        <p>
          <Term>概率抽样</Term>
          （每个单元都有一个已知的被选中机会）是让你能泛化的东西。主要的设计有：
        </p>
        <ul>
          <li>
          ...
          <li>
            <Term>系统抽样</Term>——从一份有序名单里每隔 k
            个取一个。简单，但要当心名单里隐藏的周期性。
          </li>
        </ul>
        <Callout type="pitfall">
          <p>
            与之相对的是<Term>非概率抽样</Term>
            ——便利样本、配额样本、自愿参加的网络投票——其中被选中的
            机会是未知的。这些对探索可能有用，但你<strong>无法有效地</strong>
            从它们泛化到一个总体，因为
      ...
      <KSection id="error" eyebrow="04" title="两种误差——而更糟的那种是看不见的">
        ...
        <ul>
          ...
          <li>
            ...
            数据录入错误。它<em>不</em>随样本量缩小，通常<em>不</em>
            被量化，也正是那些重大而尴尬的失败的 来源。
          </li>
        </ul>
        <Callout type="intuition">
          <p>
            陷阱在于：报告出来的「± 3%」只覆盖<strong>抽样</strong>
            误差——那小小的、诚实的、规矩的部分。
            非抽样误差——它可以让前者相形见绌——则未经测量地坐在那些误差棒之外。一个巨大的样本，给你的
            是一个<em>有偏</em>
            估计周围一个微小的误差幅度：精确地错。样本量修好抽样误差，对那种真正会让
            研究沉没的误差却毫无作为。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="bias" eyebrow="05" title="会咬人的偏差">
        ...
        <ul>
          <li>
            ...
            一部分。教科书案例：1936 年《<em>文学文摘</em>
            》的民调从汽车与电话登记册里抽了数百万人——在
            大萧条时期他们比平均更富裕——并自信地预测错了选举赢家。巨大的样本，致命的覆盖偏差。
          </li>
          <li>
            <Term>无应答偏差</Term>
            ——当不应答的人与应答的人不一样。如果忙碌的人、或不快乐的人、或注重
      ...
      <KSection id="weighting" eyebrow="06" title="用加权把它拉回形状">
        ...
        <p>
          常见的技术是<Term>事后分层</Term>和<Term>迭代比例拟合</Term>
          （raking），它们轻推加权后的边际，
          去匹配年龄、性别、地区等的普查数字。这是一种强有力的校正——而它的诚实只到一个限度为止：
          <strong>
            加权能修好你能测量、并知道总体总数的那些变量上的失衡。它修不了你没测量的那些东西上的
            偏差
          </strong>
          ，而且激进的加权会抬高方差（少数被重重加权的应答者会左右整个估计）。它是一块补丁，
          不是好抽样的替代品。
        </p>
      </KSection>
      ...
      <KSection id="size" eyebrow="07" title="样本要多大？">
        <p>
          来自<Link href="/knowledge/statistics">统计学</Link>
          页的一个愉快的惊喜：一个估计的精度，主要 取决于<em>绝对</em>
          样本量，而非占总体的比例。一个比例的误差幅度，随样本量的平方根缩小：
        </p>
        <Formula label="The margin of error is approximately z times the square root of p times one minus p divided by n.">
          {String.raw`\text{MoE} \approx z \sqrt{\frac{\hat{p}(1-\hat{p})}{n}}`}
        </Formula>
        <p>
          ...
          抽样误差已经很小，你得到的是递减的回报，而你真正应该担心的<em>非</em>
          抽样误差却纹丝不动。花钱 去做一个更大的样本来修一个有偏的样本，是经典的资源错配。
        </p>
      </KSection>
      ...
      <KSection id="applied" eyebrow="08" title="它在我工作中的体现">
        <Callout type="applied" label="这个数字有代表性吗？">
          <p>
            ...
            习惯，是在信任它们中任何一个之前，先问一句<strong>「代表什么？」</strong>。
            <strong>抽样框</strong>是什么，它漏掉了谁？选择是一种<strong>概率</strong>
            设计，还是一种无法泛化的自愿 参加？最要紧的是，<strong>无应答</strong>
            的图景如何——因为那种静悄悄的、未被量化的偏差，正是
            把一个自信的统计量变成一个误导性统计量的那一个。
          </p>
          <p>
            它也是批判性地读别人数字的透镜：一个自选样本上看起来很精确的「± 2%」是
            <strong>精确地错</strong>
            ，而一个「加权使其有代表性」的数字，其好坏只取决于它据以加权的那些变量。知道抽样在
      ...
      <KSection id="refresher" eyebrow="09" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              一个小小的<strong>随机</strong>
              样本能代表一个庞大的总体——随机性是那座让你能泛化、并给它 加上诚实误差棒的桥。
            </li>
            <li>
            ...
            <li>
              ...
              <strong>非抽样误差</strong>（覆盖、无应答、糟糕的问题）更大、不被量化，且<em>不</em>
              随规模 缩小。
            </li>
            <li>
            ...
            <li>
              要把误差幅度减半，把样本<strong>翻四倍</strong>
              。一个有偏估计周围一个更大的样本，是精确 地错。
            </li>
          </ul>
        </Callout>
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#6E6E6E] mt-6 [text-wrap:pretty]">
