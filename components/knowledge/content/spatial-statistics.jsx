...
function MoranFigure({ caption, ariaLabel, clusteredLabel, randomLabel }) {
  return (
    <Figure caption={caption}>
      ...
      >
        <text x="100" y="16" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="#FF3C3C">{clusteredLabel}</text>
        <text x="340" y="16" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="currentColor" opacity="0.7">{randomLabel}</text>
  ...
  return (
    <>
      <p>
        ...
        <strong>独立的</strong>——但彼此邻近的地方明确地<em>不</em>独立。相邻的街区收入相似，相邻的地区天气
        相似，邻近的区域犯罪率相似。<Term>空间统计</Term>是为地理要紧的数据而建的分支，它的全部目的，是认真
        对待那种空间依赖，而非假装它不存在。
      </p>
      <p>
        它与 <Link href="/knowledge/geospatial-analysis">GIS</Link> 页不同——那一页是处理空间数据的工具；这
        一页是位置的<em>推断统计学</em>：如何度量空间模式、检验一个聚集是否真实，以及在空间上做预测。这一页
        讲核心的想法，而反复出现的主题是：忽略「在哪里」会产出自信而错误的答案。
      </p>

      <KSection id="why" eyebrow="01" title="为什么地理打破规则">
        <p>
          <Link href="/knowledge/statistics">经典统计</Link>里几乎每一种方法，都建立在<strong>独立观测
          </strong>的假设之上——知道一个数据点，告诉不了你关于下一个的任何东西。空间数据公然违反这一点：知道
          一个街区的值，告诉你很多关于它邻居的事。这种<Term>空间自相关</Term>意味着你的有效样本量比它看上去
          的要小（邻近的点携带冗余的信息），所以普通的分析报告<strong>过度自信</strong>的结果——并不存在的
          显著性、被共同位置吹胀的相关。
        </p>
        <p>
          所以空间统计做两件事：它<em>度量</em>空间依赖（有没有模式，在哪里？），并在模型里<em>考虑</em>它，
      ...
      <KSection id="law" eyebrow="02" title="地理学第一定律">
        <p>
          托布勒的<Term>地理学第一定律</Term>把它说得很直白：<em>「万物彼此相关，但近的东西比远的东西更
          相关。」</em>那是整个领域的引擎。空间自相关可以是<strong>正的</strong>（常见情形——相似的值聚在
          一起，比如财富或温度），或者更罕见地，<strong>负的</strong>（高值系统性地紧挨着低值，像一张
      ...
      <KSection id="neighbours" eyebrow="03" title="定义「邻居」">
        <p>
          在你能度量空间关系之前，你必须把什么算作「近」形式化。那就是<Term>空间权重矩阵</Term>——对每一对
          位置，一个说明它们有多连通的权重。常见的选择：<em>邻接</em>（共享一条边界的地区是邻居）、
      ...
      <KSection id="moran" eyebrow="04" title="度量聚集：Moran's I">
        <p>
          ...
          <Link href="/knowledge/statistics">相关</Link>系数：它问一个位置的值是否倾向于与它邻居的值相匹配，
          在整张地图上。
        </p>
        <Formula label="Moran's I equals N over the sum of weights, times the sum over all pairs i, j of the weight times the deviation of x-i from the mean times the deviation of x-j from the mean, divided by the sum of squared deviations.">
          {String.raw`I = \frac{N}{\sum_{i}\sum_{j} w_{ij}} \cdot \frac{\sum_{i}\sum_{j} w_{ij}\,(x_i - \bar{x})(x_j - \bar{x})}{\sum_{i} (x_i - \bar{x})^2}`}
        ...
        <p>
          你不需要背公式——读它的行为。Moran's I 大致从 −1 跑到 +1：一个<strong>远高于 0</strong>的值意味着正
          自相关（聚集——相似的值彼此邻近），<strong>接近 0</strong>意味着一个随机的空间模式，而
          <strong>低于 0</strong>意味着分散。关键的是，你要检验它的显著性（对照「随机排列」的零假设），这样你
      ...
      <KSection id="lisa" eyebrow="05" title="找到热点：LISA">
        <p>
          ...
          <Term>LISA</Term>（空间关联局部指标）把这个全局的统计量分解成每个位置的一个值，揭示出局部的结构。
          它把每个区域分类为属于一个：
        </p>
        <ul>
          <li>
          ...
          <li>
            <Term>高—低 / 低—高</Term>——空间<strong>离群值</strong>：一个与其周遭对着干的值（安全地区里
      ...
      <KSection id="regression" eyebrow="06" title="空间回归：诚实的模型">
        <p>
          ...
          <Link href="/knowledge/linear-statistical-models">回归</Link>就无效了——独立性假设被打破，标准误在
          撒谎。<Term>空间回归</Term>通过把空间结构建进模型里来修这个：
        </p>
        <ul>
          <li>
            <Term>空间滞后</Term>模型——把邻居的结果值作为一个预测变量纳入，捕捉真正的溢出（一个地区的值被
            它邻居的所塑造）。
          </li>
          <li>
        ...
        <p>
          无论哪种方式，要点都与别处的<Link href="/knowledge/causal-inference">因果</Link>纪律一样：尊重数据的
          结构，否则你的自信是假的。
        </p>
      </KSection>
      ...
      <KSection id="kriging" eyebrow="07" title="在点之间预测：克里金">
        <p>
          最后一个大工具是空间<em>预测</em>。你在散落的位置上测量了一个值（气象站的降雨、采样点的一种
          污染物），想估计它<em>在中间的每一处</em>。<Term>克里金</Term>做这件事，而它比朴素的插值更聪明：它
          使用<em>测量出的</em>空间自相关结构——相似性随距离衰减得有多快——在每个未采样的点上做出统计上最优的
          预测，还附带一个不确定性估计。
        </p>
        <p>
          它是<Link href="/knowledge/time-series-analysis">时间序列</Link>页上那些预测想法的空间兄弟（在空间上
      ...
      <KSection id="applied" eyebrow="08" title="它在我工作中的体现">
        <Callout type="applied" label="真实的热点，而非海市蜃楼">
          <p>
            ...
            <strong>Moran's I</strong> 检验到底有没有真实的空间结构，而 <strong>LISA</strong> 钉住真正的热点
            （以及那些说明问题的离群值）<em>在哪里</em>——这正是你在对「它在哪里最严重？」采取行动之前所
            需要的。
          </p>
          <p>
            它还让建模保持诚实：把空间依赖的数据当作独立的来处理，会产出<strong>过度自信</strong>的结论，
            所以<strong>空间回归</strong>是阻止一项区域分析夸大它确定性的东西——与
            <Link href="/knowledge/causal-inference">因果推断</Link>同样的「尊重结构」的教训。它补足
            <Link href="/knowledge/geospatial-analysis">GIS</Link> 页（工具对推断），并共享那个关于「面积单元
            的选择如何塑造一切」的 MAUP 告诫。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="09" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              空间数据打破<strong>独立性假设</strong>——邻近的地方相关（<strong>空间自相关</strong>），所以
              朴素的统计<strong>过度自信</strong>。
            </li>
            <li>
              <strong>托布勒第一定律</strong>：近的东西比远的东西更相关。正自相关（聚集）是常见情形。
            </li>
            <li>
              通过<strong>空间权重矩阵</strong>定义<strong>邻居</strong>（邻接 / 距离 / k-最近）——一个真正的
              建模选择。
            </li>
            <li>
              <strong>Moran's I</strong> = 空间的一个相关（远高于 0 = 聚集；≈0 = 随机），检验显著性。
              <strong>LISA</strong> 找到<em>在哪里</em>——高—高热点、低—低冷点，以及离群值。
            </li>
            <li>
              <strong>空间回归</strong>（滞后/误差模型）把结构建进去，好让推断保持诚实。<strong>克里金
              </strong>用自相关在采样点之间预测，附带不确定性。
            </li>
            <li>
