    ...
    <Figure caption={caption}>
      ...
      >
        {LOOP_NODES.map(([cx, cy], i) => {
          ...
          return (
            <g key={i}>
              <rect x={cx - 62} y={cy - 15} width="124" height="30" rx="5" fill="none" stroke={hot ? "#FF3C3C" : "currentColor"} strokeWidth={hot ? "1.5" : "1.3"} />
              <text x={cx} y={cy + 4} textAnchor="middle" fontSize="9.5" fontFamily="monospace" fill={hot ? "#FF3C3C" : "currentColor"}>{nodeLabels[i]}</text>
            </g>
          );
        })}
        <line x1="152" y1="39" x2="290" y2="69" stroke="currentColor" strokeWidth="1.2" markerEnd="url(#asah)" />
        <line x1="290" y1="81" x2="152" y2="111" stroke="#FF3C3C" strokeWidth="1.2" markerEnd="url(#asahr)" />
        <line x1="90" y1="100" x2="90" y2="50" stroke="currentColor" strokeWidth="1.2" markerEnd="url(#asah)" />
        <text x="58" y="78" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.6">{retrainLabel}</text>
        <defs>
          <marker id="asah" markerWidth="7" markerHeight="7" refX="6" refY="2.5" orient="auto"><path d="M0,0 L6,2.5 L0,5 Z" fill="currentColor" /></marker>
          <marker id="asahr" markerWidth="7" markerHeight="7" refX="6" refY="2.5" orient="auto"><path d="M0,0 L6,2.5 L0,5 Z" fill="#FF3C3C" /></marker>
        </defs>
      </svg>
    </Figure>
  );
...
function ZhBody() {
  return (
    <>
      <p>
        标准的<Link href="/knowledge/statistical-machine-learning">监督学习</Link>有一副昂贵的胃口：它
        需要大量<em>带标签</em>的样本，而标注缓慢、昂贵，往往还需要一位人类专家。然而<em>无标签</em>的
        数据通常充裕、几乎免费——成堆没人标注过的文本、图像与记录。<Term>半监督</Term>与
        <Term>主动学习</Term>是针对同一个现实问题的两种策略：<strong>在标签稀缺时学出一个好模型</strong>
        ——办法是把你手里的无标签数据用到极致，或者对「哪几个样本值得付标注的代价」放聪明些。
      </p>
      <p>
        它是机器学习里一个真正独特的角落——既非完全监督、也非完全
        <Link href="/knowledge/clustering">无监督</Link>——而且在任何标签是瓶颈之处都非常实用。这一页讲
      ...
      <KSection id="why" eyebrow="01" title="标注的瓶颈">
        <p>
          ...
          那些数据的<em>无标签</em>版本却在免费地堆积。自然的问题随之而来：我们能否靠利用无标签的丰盈，
      ...
      <KSection id="semi" eyebrow="02" title="半监督：从无标签的那一堆里学">
        <p>
          <Term>半监督学习</Term>把一个<em>小的</em>带标签集<em>加上</em>一个<em>大的</em>无标签集放在
          一起训练，让无标签数据的结构来磨利模型。两种常见的机制：
        </p>
        <ul>
          <li>
            <Term>自训练 / 伪标注</Term>——在带标签数据上训练一个模型，用它为无标签数据<em>预测</em>
            标签，把其中最有把握的那些保留为<Term>伪标签</Term>，再在扩大了的集合上重新训练。模型自举
            自己，用它自己有把握的猜测来教自己。
          </li>
          <li>
            <Term>标签传播</Term>——建一张<Link href="/knowledge/network-graph-analysis">图</Link>把
            相似的点连起来，然后让那少数已知的标签沿着边<em>扩散</em>到它们无标签的邻居。标签像染料在
            水中那样流向邻近的点。
          </li>
        </ul>
      ...
      <KSection id="assumptions" eyebrow="03" title="无标签数据究竟何时有帮助？">
        <p>
          无标签数据不是魔法——只有当它的<em>结构</em>告诉你一些关于标签的事情时，它才有帮助。让那成立的
          那些假设：
        </p>
        <ul>
          <li>
            <Term>聚类假设</Term>——同一个稠密<Link href="/knowledge/clustering">簇</Link>里的点倾向于
            共享一个标签，所以无标签数据揭示出簇（从而决策边界）在哪里。
          </li>
          <li>
        ...
        <Callout type="intuition">
          <p>
            统一的直觉：无标签数据给你看数据的<em>形状</em>——哪里稠密、缝隙在哪——而那个形状提示了决策
            边界<em>不</em>该切过哪里（穿过稠密区域）。如果这些假设成立，几个标签就能走很远。如果它们
            <em>不</em>成立——如果边界确实穿过一个稠密的簇——无标签数据反而会主动误导。那份条件性，就是
            全部的难处。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="active" eyebrow="04" title="主动学习：选择标注什么">
        <p>
          <Term>主动学习</Term>从另一侧攻击成本。它不随机地标注数据，而是让<strong>模型选择哪些样本最
          值得标注</strong>——然后由一个人只标注那些。既然预算很小，把它花在<em>最有信息量</em>的点上
        ...
        <ActiveLoopFigure
          ...
          retrainLabel="重新训练"
        />
        <p>它怎么挑？常见的策略都瞄准<em>信息量</em>：</p>
        <ul>
          <li>
            <Term>不确定性采样</Term>——标注模型<em>最没把握</em>的点（在它决策边界附近）。解决那些，教它
            的最多。（知道<em>有多</em>没把握，连到<Link href="/knowledge/conformal-prediction">不确定性
            量化</Link>。）
          </li>
          <li>
      ...
      <KSection id="selfsup" eyebrow="05" title="自监督：现代的表亲">
        <p>
          值得一提，因为它驱动着当今最大的那些模型：<Term>自监督学习</Term>通过一个<em>前置任务</em>，从
          无标签数据本身制造标签——从其余部分预测输入被藏起来的一部分（预测下一个词；补全一张图像被遮
          住的一块）。完全没有人类标签，模型却学到丰富、通用的表示，之后可以在一个小的带标签集上微调。
          这正是 <Link href="/knowledge/large-language-models">LLM</Link> 被预训练的方式（下一个词元的
      ...
      <KSection id="pitfalls" eyebrow="06" title="它在哪里出错">
        ...
        <Callout type="pitfall">
          <p>
            半监督学习的危险是<Term>确认偏误</Term>——伪标注会<em>放大它自己的错误</em>。如果模型自信地给
            一些无标签的点贴错标签、又在那些错误的标签上训练，它就会变得更自信地错，陷入一个自我强化的
            螺旋。主动学习的危险是<strong>采样偏差</strong>：通过刻意只标注那些不寻常的、不确定的点，带
            标签的集合就不再能代表总体——这会扭曲评估、乃至模型本身。而两者都依赖于一些<strong>假设
            </strong>（聚类/平滑性），当这些假设被违反时，会让无标签数据<em>有害</em>而非有益。教训是：在
      ...
      <KSection id="applied" eyebrow="07" title="它在我工作中的体现">
        <Callout type="applied" label="当带标签的个案稀少时">
          <p>
            ...
            的记录则很充裕。那恰恰是这些方法为之而建的场景。<strong>主动学习</strong>是更直接有用的那
            一个：当一位专家标注个案的时间是瓶颈时，把它花在模型<strong>最不确定</strong>的个案、而非
            一个随机样本上，就能用少得多的标签得到一个可用的分类器。
          </p>
          <p>
            让它保持诚实的，是那些失败模式——伪标签的<strong>确认偏误</strong>（模型放大自己的错误）与
            主动学习的<strong>采样偏差</strong>（一个不再代表总体的带标签集）。两者都意味着结果需要在一个
            干净的留出集上做小心的<Link href="/knowledge/model-evaluation">验证</Link>。它连到
            <Link href="/knowledge/statistical-machine-learning">监督/无监督</Link>的划分、
            <Link href="/knowledge/clustering">聚类</Link>（那些假设），以及贯穿那些负责任 AI 页的人在
            回路的纪律。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="08" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              <strong>半监督</strong>：在一点点带标签 + 大量无标签上训练——<strong>伪标注</strong>（在有
              把握的猜测上自训练）与<strong>标签传播</strong>（沿一张相似度图扩散标签）。
            </li>
            <li>
              只有假设成立才有帮助——<strong>聚类 / 流形 / 平滑性</strong>（无标签数据揭示边界不该切过
              哪里）。否则它会误导。
            </li>
            <li>
              <strong>主动学习</strong>：模型<em>选择</em>标注什么——<strong>不确定性采样</strong>（标注它
              最没把握的）、委员会查询。一个人在回路的查询循环。
            </li>
            <li>
              <strong>自监督</strong>学习从数据本身制造标签（前置任务）——LLM 被预训练的方式。
            </li>
            <li>
              陷阱：伪标签的<strong>确认偏误</strong>（放大错误）、主动学习的<strong>采样偏差</strong>
