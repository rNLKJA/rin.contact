  ...
  return (
    <Figure caption={caption}>
      ...
      >
        ...
        <path
          d="M300 98 C 340 98, 345 50, 365 50 C 385 50, 390 98, 430 98"
          fill="none"
          stroke="#FF3C3C"
          strokeWidth="1.5"
        />
...
function EnBody() {
  return (
    <>
      <p>
        Classical statistics gives beautiful closed-form answers — but only for the problems neat
        enough to have them. The standard error of a mean, the formula for a confidence interval:
        these exist because someone could do the algebra. The moment your statistic is unusual, your
        model is complex, or your assumptions don't hold, the algebra runs out.{" "}
        <Term>Computational statistics</Term> is the answer: when you can't <em>derive</em> the
        result, <em>compute</em> it — by simulation, resampling, and iteration.
      </p>
      <p>
        It's the bridge from the <Link href="/knowledge/statistics">statistics page's</Link> theory
        to what actually runs on real, messy data, and it's the machinery under{" "}
        <Link href="/knowledge/bayesian-statistics">Bayesian</Link> inference. The unifying idea is
        simple and a little subversive: trade mathematical elegance for brute-force computing, and
        let the computer find the answer the maths couldn't.
      </p>

      <KSection id="why" eyebrow="01" title="When the maths runs out">
        <p>
          The classical formulas of the <Link href="/knowledge/statistics">statistics page</Link>{" "}
          rest on assumptions — usually that data is normally distributed and your statistic is
          something simple like a mean. Reality routinely breaks both: a skewed distribution, a
          small sample, or a quantity like a median, a correlation, or a ratio for which no tidy
          standard-error formula exists.
        </p>
        <p>
          Rather than give up (or pretend the assumptions hold), computational statistics reframes
          the question. Instead of <em>deriving</em> how a statistic behaves, you <em>simulate</em>{" "}
          it — generate the relevant randomness many times and watch what happens. With enough
          computing, the empirical answer is as good as the analytic one would have been, and it
          works for problems no formula can reach.
        </p>
      </KSection>

      <KSection id="montecarlo" eyebrow="02" title="Monte Carlo methods">
        <p>
          The foundational technique is the <Term>Monte Carlo</Term> method: estimate a quantity you
          can't compute directly by drawing many random samples and averaging. Want the expected
          value of some function of a random variable? Don't integrate — sample it{" "}
          <TeX>{TEX.N}</TeX> times and take the mean:
        </p>
        <Formula
        ...
        <p>
          It works because of the <Link href="/knowledge/probability">Law of Large Numbers</Link> —
          averages converge to the truth as samples accumulate — and its error shrinks at a
          predictable rate of <TeX>{TEX.invSqrtN}</TeX>, so to halve the error you need four times
          the samples. From estimating π by throwing random darts at a square, to pricing financial
          options, to integrating in high dimensions where ordinary numerical integration collapses,
          Monte Carlo is the workhorse. The price is compute; the reward is answers to otherwise
      ...
      <KSection id="bootstrap" eyebrow="03" title="The bootstrap">
        <p>
          The single most useful idea in computational statistics is the <Term>bootstrap</Term>, and
          it sounds like cheating. You want to know how much a statistic (say, a median) would vary
          if you could collect many fresh samples — but you only have <em>one</em> sample. The
          bootstrap's trick: treat your sample as if it were the population, and draw new samples{" "}
          <em>from it</em>.
        </p>
        <ul>
          <li>
            ...
            <Term>
              resample <TeX>{TEX.n}</TeX> points with replacement
            </Term>{" "}
            — some originals appear twice, some not at all.
          </li>
          <li>Compute your statistic on this bootstrap sample.</li>
          <li>
            Repeat thousands of times. The <em>spread</em> of those values estimates the statistic's
            standard error, and their percentiles give a confidence interval.
          </li>
        </ul>
        <p>
          That's it — and it works for <em>any</em> statistic, with no formula and no distributional
          assumptions. It gives you the standard errors and{" "}
          <Link href="/knowledge/statistics">confidence intervals</Link> from the statistics page
          for quantities that have no analytic ones. Resampling your single dataset really does
          reveal how much your estimate would have wobbled — one of those rare ideas that feels too
          good to be true and simply isn't.
        </p>
      ...
      <KSection id="permutation" eyebrow="04" title="Permutation tests">
        <p>
          ...
          <Term>permutation test</Term> asks "is the difference between these two groups real, or
          could it be chance?" by simulating chance directly: if the group labels truly didn't
          matter (the null hypothesis), you could shuffle them and see no difference.
        </p>
        <p>
      ...
      <KSection id="em" eyebrow="05" title="The EM algorithm">
        <p>
          ...
          variable you'd need to know to estimate the parameters, but you'd need the parameters to
          figure out the latent variable. The classic case is a mixture of groups — you want each
          group's parameters, but you don't know which group each point belongs to.
        </p>
        <p>
        ...
        <ul>
          <li>
            <Term>E-step</Term> — given the current parameters, estimate the hidden variable (e.g.
            each point's probability of belonging to each group).
          </li>
          <li>
            <Term>M-step</Term> — given those estimates, update the parameters to their best values.
          </li>
        </ul>
        <p>
          Each round increases the likelihood, so it converges. It's the engine behind Gaussian
          mixture models and many <Link href="/knowledge/clustering">clustering</Link> and
          missing-data methods — and it's the same alternate-and-converge pattern as{" "}
          <Link href="/knowledge/clustering">k-means</Link>, which is essentially a hard version of
          EM.
        </p>
      </KSection>
      ...
      <KSection id="optimisation" eyebrow="06" title="Numerical optimisation">
        <p>
          ...
          <Link href="/knowledge/statistics">maximum-likelihood</Link> parameters — and those rarely
          have a closed-form solution. So you solve them numerically, with the{" "}
          <Link href="/knowledge/calculus-optimisation">gradient-based methods</Link> from the
          calculus page: start somewhere, follow the slope toward the optimum, iterate. Fitting a{" "}
          <Link href="/knowledge/statistical-modelling">GLM</Link>, a complex likelihood, or almost
          any modern model is exactly this under the hood — the computer hill-climbing to the best
          parameters because no formula hands them over.
        </p>
      </KSection>
      ...
      <KSection id="mcmc" eyebrow="07" title="MCMC, in one line">
        <p>
          ...
          <Link href="/knowledge/bayesian-statistics">Bayesian page</Link>, but it belongs here too:{" "}
          <Term>Markov Chain Monte Carlo</Term> is computational statistics solving the hardest
          problem of all — drawing samples from a distribution you only know up to a constant, so
          you can compute with a posterior that has no closed form. It's the reason Bayesian
          inference became practical, and it's Monte Carlo and simulation taken to their logical
          end.
        </p>
      </KSection>
      ...
      <KSection id="randomness" eyebrow="08" title="The catch: randomness">
        <p>
          All of this leans on a steady supply of random numbers — and computers are deterministic,
          so they can't make truly random ones. They use <Term>pseudo-random</Term> generators:
          algorithms that produce sequences statistically indistinguishable from random but fully
          determined by a starting <Term>seed</Term>. That's a feature, not a bug:{" "}
          <strong>set the seed and your "random" simulation is exactly reproducible</strong> — the
          same <Link href="/knowledge/elements-of-data-processing">reproducibility</Link> discipline
          as everywhere else, applied to randomness itself. (The other practical catch is cost: more
          samples means more compute, so techniques like <em>variance reduction</em> aim to get the
          same accuracy from fewer draws.)
        </p>
      </KSection>
      ...
      <KSection id="applied" eyebrow="09" title="Where it shows up in my work">
        <Callout type="applied" label="When the formula doesn't exist">
          <p>
            ...
            formula, and resampling gives me an honest <strong>confidence interval</strong> for any
            of them without pretending the data is normal. <strong>Simulation</strong> more broadly
            is how I stress-test an idea when the maths is intractable: generate data under
            assumptions and see what happens, rather than wave hands.
          </p>
          <p>
            The mindset is the useful part — when classical theory doesn't fit the messy reality in
            front of me, I don't force it; I compute the answer instead. And I{" "}
      ...
      <KSection id="refresher" eyebrow="10" title="Refresh in 60 seconds">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            <li>
              When the maths has no closed form, <strong>compute</strong> the answer by simulation
              and resampling instead of deriving it.
            </li>
            <li>
              <strong>Monte Carlo</strong>: estimate by sampling — <TeX>{TEX.mcShort}</TeX>; error
              shrinks like <TeX>{TEX.invSqrtN}</TeX>.
            </li>
            <li>
              The <strong>bootstrap</strong>: resample your one dataset with replacement thousands
              of times → standard errors &amp; CIs for <em>any</em> statistic, no assumptions.
            </li>
            <li>
              <strong>Permutation tests</strong>: shuffle labels to simulate the null → a p-value
              from first principles.
            </li>
            <li>
              <strong>EM</strong>: alternate E-step (estimate hidden variable) and M-step (update
              parameters) for latent-variable models. <strong>Numerical optimisation</strong> fits
              most models (MLE by gradient methods).
            </li>
            <li>
              It all runs on <strong>pseudo-random</strong> numbers — <strong>set the seed</strong>{" "}
              for reproducibility. <strong>MCMC</strong> is this taken to its limit (Bayesian
              posteriors).
            </li>
          </ul>
        </Callout>
  ...
  return (
    <>
      ...
      <KSection id="bootstrap" eyebrow="03" title="自助法">
        ...
        <ul>
          <li>
            从你那有 <TeX>{TEX.n}</TeX> 个点的数据集里，
            <Term>
              有放回地重采样 <TeX>{TEX.n}</TeX> 个点
            </Term>
            ——有些原始点出现两次，有些一次都不出现。
          </li>
      ...
      <KSection id="randomness" eyebrow="08" title="症结：随机性">
        <p>
          这一切都倚赖随机数的稳定供应——而计算机是确定性的，所以它们造不出真正随机的数。它们 使用
          <Term>伪随机</Term>生成器：产生在统计上与随机不可区分、却完全由一个起始
          <Term>种子</Term>决定的序列的算法。这是特性，而非缺陷：
          <strong>设定种子，你那「随机」 的模拟就完全可复现</strong>——与别处一样的
