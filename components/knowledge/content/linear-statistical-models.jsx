  ...
  return (
    <>
      ...
      <KSection id="why" eyebrow="01" title="人人都会拿来用的模型">
        <p>
          线性回归回答了一个看似简单、实则丰富的问题：当一些输入 <code>x</code> 变化时，
          结果 <code>y</code> 平均会如何变化——我们又有多确定？根据面积与地段预测房价、
      ...
      <KSection id="model" eyebrow="02" title="线性模型">
        ...
        <p>
          ...
          （每行一个观测，每列一个特征，外加一列全为 1 用于截距），<code>β</code> 是我们想
          学习的<Term>系数</Term>向量，<code>ε</code> 是<Term>误差项</Term>——特征无法解释的
          一切。「线性」指的是<em>对系数而言</em>是线性的；你仍然可以通过向 <code>X</code>{" "}
          添加 <code>x²</code> 或交互项列来拟合曲线，这正是它远比初看更灵活的原因。
        </p>
      </KSection>

      <KSection id="ols" eyebrow="03" title="普通最小二乘法">
        <p>
          要拟合模型，你需要找到让直线最贴近数据的 <code>β</code>。
          <Term>普通最小二乘法</Term>（OLS）把「最贴近」定义为最小化<em>残差平方</em>之和
      ...
      <KSection id="geometry" eyebrow="04" title="OLS 的几何">
        ...
        <p>
          OLS 找出子空间中<em>最接近</em> <code>y</code> 的点——而最近的点正是{" "}
          <code>y</code> 在其上的<Term>正交投影</Term>。预测值 <code>ŷ</code> 就是这个投影，
          残差 <code>y − ŷ</code> 与子空间垂直。这就是最小二乘法奏效的原因：最小化平方距离
      ...
      <KSection id="assumptions" eyebrow="05" title="那些假设">
        ...
        <ul>
          ...
          <li>
            <Term>无完全多重共线性</Term>——没有任何特征是其他特征的精确组合
            （这样 <code>XᵀX</code> 才可逆）。
          </li>
        </ul>
        <p>
      ...
      <KSection id="refresher" eyebrow="10" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              <strong>OLS</strong> 最小化残差平方；闭式解{" "}
              <code>β̂ = (XᵀX)⁻¹Xᵀy</code>。几何上它是 y 在列空间上的
              <strong>正交投影</strong>。
            </li>
            <li>
