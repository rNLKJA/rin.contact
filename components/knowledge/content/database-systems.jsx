  ...
  return (
    <Figure caption={caption}>
      ...
      >
        ...
        <text x="220" y="30" textAnchor="middle" fontSize="10" fontFamily="monospace" fill="currentColor">{rootLabel}</text>
        {[70, 220, 370].map((x, i) => (
          <g key={i}>
            <line x1="220" y1="37" x2={x} y2="65" stroke="currentColor" strokeWidth="1" opacity={i === 2 ? 1 : 0.4} />
        ...
        <text x="410" y="124" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="#FF3C3C">{rowLabel}</text>
        <text x="120" y="128" fontSize="9" fontFamily="monospace" fill="currentColor" opacity="0.6">{hopsLabel}</text>
      </svg>
    </Figure>
  );
}
  ...
  return (
    <>
      ...
      <KSection id="relational" eyebrow="02" title="关系模型">
        <p>
          占主导地位的设计，其要旨自 1970 年代以来未变，就是<Term>关系模型</Term>：把数据
          存在<Term>表</Term>（关系）中，每一行是一条记录，每一列是一个属性。表与表之间由
          <Term>键</Term>相连：
        </p>
        <ul>
          <li>
      ...
      <KSection id="sql" eyebrow="03" title="SQL：向数据发问">
        <p>
          ...
          「给我每个区域的总销售额，从高到低排序」，数据库的查询规划器便算出最高效的计算
          方式。
        </p>
        <pre><code>{SQL}</code></pre>
        <p>
      ...
      <KSection id="joins" eyebrow="05" title="连接（JOIN）">
        <p>
          ...
          <code>orders.customer_id = customers.id</code> 上把 orders 连接到 customers。
          主要的几种：
        </p>
        <ul>
          <li>
      ...
      <KSection id="indexing" eyebrow="06" title="索引">
        <p>
          ...
          一样——你不会读每一页。一个<Term>索引</Term>是一个独立的、有序的数据结构（几乎
          总是一棵 <Term>B 树</Term>），它让数据库近乎瞬间地跳到匹配的行，而不必扫描整张表。
        </p>
        <p>
      ...
      <KSection id="oltp" eyebrow="08" title="OLTP、OLAP 与 NoSQL">
        <p>
          并非所有数据库都为同一种工作而调优，而把存储与工作负载匹配起来是一个真实的设计
          决策：
        </p>
        <ul>
          <li>
            <Term>OLTP</Term>（事务型）——许多小而快的读写；规范化；应用背后的实时系统。
            为 ACID 吞吐而优化。
          </li>
          <li>
            <Term>OLAP</Term>（分析型）——对历史数据的大型聚合查询；常被反规范化进一个采用
            列式存储的<Term>数据仓库</Term>。这正是大多数分析与 BI 实际运行之处。
          </li>
          <li>
      ...
      <KSection id="applied" eyebrow="09" title="它在我工作中的体现">
        <Callout type="applied" label="每一次分析之下的技能">
          <p>
            ...
            是我如何从一个规范化的 <strong>OLTP</strong> 源组装数据而不悄悄丢行或重复行；而
            理解 <strong>OLTP/OLAP</strong> 的划分，正是我为快速聚合读取而建模一个 Power BI
            数据层、而非去查询实时事务系统的原因。
          </p>
          <p>
