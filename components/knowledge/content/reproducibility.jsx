...
function PipelineFigure({ caption, ariaLabel, manualHeader, pipelineHeader, manualLabels, pipelineLabels, footer }) {
  return (
    <Figure caption={caption}>
      ...
      >
        <text x="14" y="32" fontSize="11" fontFamily="monospace" fill="#FF3C3C">✗</text>
        <text x="30" y="18" fontSize="9" fontFamily="monospace" fill="#FF3C3C">{manualHeader}</text>
        {manualLabels.map((t, i) => (
          <g key={`m${i}`}>
            <rect x={30 + i * 84} y="24" width="64" height="22" rx="3" fill="none" stroke="#FF3C3C" strokeWidth="1.2" strokeDasharray="3 2" />
            <text x={62 + i * 84} y="39" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="currentColor">{t}</text>
          </g>
        ))}
        <text x="14" y="118" fontSize="11" fontFamily="monospace" fill="currentColor">✓</text>
        <text x="30" y="104" fontSize="9" fontFamily="monospace" fill="currentColor">{pipelineHeader}</text>
        {pipelineLabels.map((t, i) => (
          <g key={`p${i}`}>
            <rect x={30 + i * 84} y="110" width="64" height="22" rx="3" fill="none" stroke="currentColor" strokeWidth="1.3" />
            <text x={62 + i * 84} y="125" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="currentColor">{t}</text>
            {i < 4 && <line x1={94 + i * 84} y1="121" x2={114 + i * 84} y2="121" stroke="currentColor" strokeWidth="1.2" markerEnd="url(#rah)" />}
          </g>
        ))}
        <text x="30" y="156" fontSize="8.5" fontFamily="monospace" fill="currentColor" opacity="0.6">{footer}</text>
        <defs>
          <marker id="rah" markerWidth="7" markerHeight="7" refX="6" refY="2.5" orient="auto"><path d="M0,0 L6,2.5 L0,5 Z" fill="currentColor" /></marker>
        </defs>
      </svg>
    </Figure>
  );
...
function EnBody() {
  return (
    <>
      <p>
        Here's a test that quietly separates good analysis from fragile analysis: if someone handed you
        the same raw data in a year, could you reproduce your exact result — the same number, the same
        chart, the same conclusion? For a surprising amount of real-world work the honest answer is
        "no", because the result lived in a tangle of manual steps, hand-edited spreadsheets, and a
        notebook run out of order that nobody could re-run today.
      </p>
      <p>
        <Term>Reproducibility</Term> is the discipline of making sure you <em>can</em> — that an
        analysis is a repeatable, auditable process rather than a one-off act of craft. It's the least
        glamorous topic in this whole section and, in any setting where the work has to be defended, one
        of the most important. This page is the practical kit for getting there.
      </p>

      <KSection id="why" eyebrow="01" title="Could you re-run it?">
        <p>
          It helps to separate two related ideas. <Term>Reproducibility</Term> means: same data, same
          code, same result — anyone can re-run your analysis and get what you got. (The stronger{" "}
          <Term>replicability</Term> means a fresh study reaches the same conclusion.) Reproducibility
          is the achievable, foundational one, and it buys you several things at once:
        </p>
        <ul>
          <li>
          ...
          <li>
            <strong>Auditability</strong> — when someone asks "how did you get this?", you can show the
            exact path from raw data to number.
          </li>
          <li>
            <strong>Maintainability</strong> — when the data refreshes next quarter, you re-run rather
            than rebuild from memory.
          </li>
          <li>
            <strong>Collaboration</strong> — including with your future self, who will remember none of
            today's undocumented decisions.
          </li>
        </ul>
      </KSection>
      ...
      <KSection id="crisis" eyebrow="02" title="Why it's a real problem">
        <p>
          ...
          code weren't preserved in a runnable state. Studies re-running published analysis notebooks
          have found that many simply fail to execute top to bottom.
        </p>
        <p>
          The usual culprits are mundane and entirely avoidable: a notebook whose cells were run out of
          order, a manual edit nobody recorded, a dependency that silently updated, a file path that only
          existed on one laptop. None is dramatic; together they make work impossible to reconstruct.
          The good news is that the fixes are equally mundane — a handful of habits, below, remove almost
          all of it.
        </p>
      </KSection>
      ...
      <KSection id="version" eyebrow="03" title="Version control: the foundation">
        <p>
          <Term>Version control</Term> (Git is the standard) tracks every change to your code over time:
          what changed, when, by whom, and why. It's the single highest-value habit in the list, because
          it turns "the analysis" from a mutable pile of files into a recorded history you can return to
          any point of.
        </p>
        <Callout type="pitfall">
          <p>
            ...
            execution and hidden state — cell 8 might depend on a variable from cell 3 that you've since
            deleted, so the saved output no longer matches a clean run. For anything that has to be
            reproducible, promote the logic into{" "}
            <strong>plain, version-controlled scripts</strong> that run start to finish, and treat the
            notebook as the scratchpad it's good at being.
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="environment" eyebrow="04" title="Environments: beating 'works on my machine'">
        <p>
          ...
          <em>silently produce different numbers</em> after a library updates. "It works on my machine"
          is the sound of an un-reproducible analysis.
        </p>
        <p>
          ...
          whole computational environment. Then "same code" really does mean same code, running the same
          way.
        </p>
      </KSection>
      ...
      <KSection id="pipeline" eyebrow="05" title="Pipelines, not manual steps">
        <p>
          The deepest shift is to stop thinking of analysis as a sequence of things <em>you do</em> and
          start thinking of it as a <Term>pipeline</Term> — a coded, automated path from raw data to
          final output, where every step is a script and nothing is touched by hand. Raw data in, report
          out, one command, no manual intervention.
        </p>
        <PipelineFigure
        ...
        />
        <p>
          This is the idea behind <Term>Reproducible Analytical Pipelines</Term> (RAP), a movement that
          began in government precisely to make official statistics auditable and re-runnable. A good
          pipeline is also <Term>idempotent</Term> — run it twice and you get the same result, with no
          leftover state from last time mucking up the next run.
        </p>
      </KSection>
      ...
      <KSection id="determinism" eyebrow="06" title="Seeds & determinism">
        <p>
          ...
          quietly breaks reproducibility. The fix is a <Term>random seed</Term>: fix the seed and the
          "random" sequence becomes identical every run, so results are exactly repeatable while still
          being statistically valid. Set it once, record it, and an entire class of "why did the number
          change?" mysteries disappears.
        </p>
      </KSection>

      <KSection id="docs" eyebrow="07" title="Documentation & data lineage">
        <p>
          Finally, code that runs isn't the same as code that's understandable. The last mile is making
          the <em>decisions</em> legible:
        </p>
        <ul>
          <li>
            <strong>A README</strong> — what this does, how to run it, what it expects. The first thing
            your future self will look for.
          </li>
          <li>
          ...
          <li>
            ...
            transformation applied to it. This is the analyst's <em>chain of custody</em>, and it ties
            straight to <Link href="/knowledge/data-governance">data governance</Link>: a number you can
            trace is a number you can defend.
          </li>
        </ul>
      </KSection>
      ...
      <KSection id="applied" eyebrow="08" title="Where it shows up in my work">
        <Callout type="applied" label="A number you can defend">
          <p>
            In a government setting, analysis isn't done when the number is produced — it's done when the
            number can be <strong>re-run, audited, and defended</strong>, sometimes long after the fact
            and sometimes by someone else entirely. That makes reproducibility a core professional
            obligation, not a nicety. The habits on this page — <strong>version control</strong>,{" "}
            <strong>pinned environments</strong>, a <strong>coded pipeline</strong> instead of
            hand-edited steps, a fixed <strong>seed</strong>, and clear <strong>lineage</strong> — are
            exactly what turns "I got this number once" into "I can show you precisely how this number
            was made."
          </p>
          <p>
            ...
            <Link href="/knowledge/data-governance">governance</Link> only count if the path from raw
            data to conclusion is recorded and re-runnable. Trustworthy analysis and reproducible
            analysis are, in the end, the same thing.
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="09" title="Refresh in 60 seconds">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              The <strong>reproducibility crisis</strong> is real — mostly mundane causes (out-of-order
              notebooks, unrecorded edits, drifting dependencies, one-laptop paths).
            </li>
            <li>
            ...
            <li>
              <strong>Pin the environment</strong> (lockfiles, Docker) to beat "works on my machine".
            </li>
            <li>
        ...
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#6E6E6E] mt-6 [text-wrap:pretty]">
          The reproducibility-crisis framing, notebook cautions, environment pinning, and Reproducible
          Analytical Pipelines (RAP) reflect current references on reproducible research and government
          analysis alongside hands-on work.
        </p>
      </KSection>
    </>
  );
    ...
    <>
      ...
      <KSection id="why" eyebrow="01" title="你能重跑它吗？">
        <p>
          ...
          结果——任何人都能重跑你的分析、得到你所得到的。（更强的<Term>可重复性</Term>意味着一项全新的
        ...
        <ul>
          ...
          <li>
            <strong>可审计</strong>——当有人问「你是怎么得出这个的？」，你能展示从原始数据到数字的确切
            路径。
          </li>
          <li>
      ...
      <KSection id="version" eyebrow="03" title="版本控制：地基">
        ...
        <Callout type="pitfall">
          <p>
            一个值得点名的具体陷阱：<strong>把笔记本当作最终产物。</strong>笔记本（Jupyter 之类）用来
            探索很棒，但它们鼓励乱序执行与隐藏状态——单元格 8 可能依赖于单元格 3 里一个你后来删掉的
            变量，于是保存的输出不再与一次干净的运行相符。对任何必须可复现的东西，把逻辑提升进
            <strong>朴素的、受版本控制的、从头跑到尾的脚本</strong>，并把笔记本当作它擅长的那个草稿本。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="environment" eyebrow="04" title="环境：打败「在我机器上能跑」">
        <p>
          代码不在真空里运行——它依赖于一个特定的 Python 或 R 版本、以及特定的包版本，而这些会变。一份
          去年能跑的分析，在一个库更新之后可能损坏，或更糟，<em>悄悄产生不同的数字</em>。「在我机器上
          能跑」是一份不可复现的分析发出的声音。
        </p>
        <p>
      ...
      <KSection id="determinism" eyebrow="06" title="种子与确定性">
        <p>
          许多方法使用随机性——一次训练/测试<Link href="/knowledge/causal-inference">拆分</Link>、一次{" "}
          <Link href="/knowledge/clustering">k-means</Link> 初始化、一次自助、一个神经网络的初始权重。
          跑它们两次你会得到略有不同的答案，而这悄悄地破坏了可复现性。修法是一个<Term>随机种子</Term>：
      ...
      <KSection id="docs" eyebrow="07" title="文档与数据血缘">
        <p>最后，能跑的代码与可被理解的代码不是一回事。最后一公里是让那些<em>决定</em>变得可读：</p>
        <ul>
          <li>
          ...
          <li>
            <strong>一本数据字典</strong>——每个字段是什么意思、它的单位、它的有效值。这里的含糊，正是
            误解悄悄溜进来之处。
          </li>
          <li>
            <strong>数据血缘 / 溯源</strong>——数据从何而来、以及施加于它的每一次转换。这是分析师的
            <em>保管链</em>，它直接连到<Link href="/knowledge/data-governance">数据治理</Link>：一个你
            能追溯的数字，是一个你能辩护的数字。
          </li>
        </ul>
      </KSection>
      ...
      <KSection id="applied" eyebrow="08" title="它在我工作中的体现">
        <Callout type="applied" label="一个你能辩护的数字">
          <p>
            在政府的环境里，分析并不在数字产出时完成——而在数字能被<strong>重跑、审计、辩护</strong>时
            完成，有时是事后很久，有时由完全另一个人来做。这让可复现性成为一项核心的职业义务，而非
            锦上添花。这一页上的习惯——<strong>版本控制</strong>、<strong>锁定的环境</strong>、一条
            <strong>编码的流水线</strong>而非手工编辑的步骤、一个固定的<strong>种子</strong>、以及清晰
            的<strong>血缘</strong>——正是把「我曾得出这个数字一次」变成「我能确切地展示这个数字是如何
            造出来的」的东西。
          </p>
          <p>
            它是本板块其他一切之下的运作脊梁：<Link href="/knowledge/feature-engineering">数据准备
            </Link>、<Link href="/knowledge/causal-inference">评估</Link>与
            <Link href="/knowledge/data-governance">治理</Link>，只有当从原始数据到结论的路径被记录、
      ...
      <KSection id="refresher" eyebrow="09" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              <strong>版本控制（Git）</strong>是地基。当心「把笔记本当作最终产物」的陷阱——把逻辑提升进
              可运行的脚本。
            </li>
            <li>
            ...
            <li>
              构建一条<strong>编码的流水线</strong>（原始 → 清洗 → 建模 → 报告，一条命令，<strong>幂等
              </strong>）——RAP 的想法——而非手工步骤。固定一个<strong>随机种子</strong>。
            </li>
            <li>
