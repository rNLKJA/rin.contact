...
function BayesLoopFigure({ caption, ariaLabel, priorLabel, bayes1, bayes2, posteriorLabel, dataLabel, loopLabel }) {
  return (
    <Figure caption={caption}>
      ...
      >
        ...
        <text x="65" y="83" textAnchor="middle" fontSize="11" fontFamily="monospace" fill="currentColor">{priorLabel}</text>
        <rect x="175" y="55" width="90" height="48" rx="2" fill="#FF3C3C" fillOpacity="0.1" stroke="#FF3C3C" strokeWidth="1.4" />
        <text x="220" y="75" textAnchor="middle" fontSize="10" fontFamily="monospace" fill="currentColor">{bayes1}</text>
        <text x="220" y="90" textAnchor="middle" fontSize="10" fontFamily="monospace" fill="currentColor">{bayes2}</text>
        <rect x="330" y="60" width="90" height="38" rx="2" fill="none" stroke="#FF3C3C" strokeWidth="1.4" />
        <text x="375" y="83" textAnchor="middle" fontSize="11" fontFamily="monospace" fill="currentColor">{posteriorLabel}</text>
        <rect x="175" y="120" width="90" height="32" rx="2" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.7" />
        <text x="220" y="140" textAnchor="middle" fontSize="9" fontFamily="monospace" fill="currentColor">{dataLabel}</text>
        ...
        <text x="220" y="158" textAnchor="middle" fontSize="8" fontFamily="monospace" fill="currentColor" opacity="0.55">{loopLabel}</text>
        <defs>
  ...
  return (
    <>
      ...
      <KSection id="idea" eyebrow="01" title="作为信念的概率">
        ...
        <p>
          那个分布正是全部要点。贝叶斯派不把它坍缩成一个猜测，而是把不确定性的<em>完整
          形状</em>带过每一次计算。当新数据到来时，分布变得更尖锐。你永远不会不再拥有一个
      ...
      <KSection id="engine" eyebrow="02" title="更新的引擎">
        <p>
          贝叶斯法则就是把先验变成后验的机器。以 <TeX>{String.raw`\theta`}</TeX> 记未知
          参数、<TeX>{String.raw`D`}</TeX> 记观测数据：
        </p>
        <Formula
        ...
        <p>
          ...
          被压制。因为分母 <TeX>{String.raw`p(D)`}</TeX> 不依赖于{" "}
          <TeX>{String.raw`\theta`}</TeX>，它只是一个把一切重新缩放到总和为一的常数——这就是
          为什么这条法则以它的<em>比例</em>形式记起来最有用：
        </p>
        <Formula label="后验正比于似然乘以先验。">
          {TEX.prop}
        </Formula>
      </KSection>
      ...
      <KSection id="worked" eyebrow="04" title="一个推演实例：硬币">
        <p>
          ...
          <TeX>{String.raw`\theta`}</TeX>——它正面朝上的概率。你抛它{" "}
          <TeX>{String.raw`n`}</TeX> 次，看到 <TeX>{String.raw`k`}</TeX> 次正面。
        </p>
        <p>
        ...
        <p>
          <strong>似然。</strong>对于一个给定的偏向，在 <TeX>{String.raw`n`}</TeX> 次抛掷中
          看到 <TeX>{String.raw`k`}</TeX> 次正面的概率，是<Term>二项</Term>似然{" "}
          <TeX>{String.raw`\theta^{k}(1-\theta)^{n-k}`}</TeX>。
        </p>
        <p>
        ...
        <p>
          后验是<em>另一个</em> Beta 分布——你只需把观测到的正面加到{" "}
          <TeX>{String.raw`\alpha`}</TeX> 上、把观测到的反面加到{" "}
          <TeX>{String.raw`\beta`}</TeX> 上。当后验像这样与先验有相同的形式时，这个先验被
          称为<Term>共轭</Term>的，更新就是纯粹的算术。从{" "}
          <TeX>{String.raw`\text{Beta}(1,1)`}</TeX> 出发，10 次中抛出 8 次正面，你的信念就
          变成 <TeX>{String.raw`\text{Beta}(9, 3)`}</TeX>——峰值在 0.75 附近，但仍诚实地保留
          不确定。
        </p>
        <Callout type="intuition">
          <p>
            ...
            方式。「你现在所相信的」与「你曾相信的加上今天的数据」，其数学字面上就是同一个
            运算。
          </p>
        </Callout>
      ...
      <KSection id="priors" eyebrow="05" title="选择先验">
        ...
        <ul>
          ...
          <li>
            ...
            <TeX>{String.raw`\text{Beta}(1,1)`}</TeX>），让数据主导。一个常见而诚实的默认
            选择。
          </li>
        </ul>
        <Callout type="pitfall">
          <p>
            ...
            先验，证明结论是稳定的。一个公开陈述的先验，比每项分析本就携带的那些隐藏假设更
            诚实。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="credible" eyebrow="06" title="可信区间 vs 置信区间">
        <p>
          一旦你有了一个后验分布，概括它既容易、又终于——直观。一个 <Term>95% 可信区间
          </Term>是任何包含后验概率 95% 的区间，而它恰好意味着人们<em>希望</em>置信区间所
          表达的：
        </p>
        <Formula label="在给定数据下，θ 落在 a 与 b 之间的概率等于 0.95。">
          {TEX.credible}
        </Formula>
        <p>
          「在给定数据下，参数有 95% 的概率落在这个区间里」——一个关于参数的直接陈述。对比
          频率派的<Link href="/knowledge/statistics">置信区间</Link>，它的 95% 是关于长期
      ...
      <KSection id="applied" eyebrow="09" title="它在我工作中的体现">
        <Callout type="applied" label="以世界真正运作的方式来推理">
          <p>
            ...
            的良好分析真正的感觉，哪怕我并没有在写一个正式的模型。每当<strong>数据稀少
            </strong>而先验知识确实值点什么（早期实验、稀有事件）时，它就是正确的框架；每当
          ...
          <p>
            ...
            同样的「先验乘以似然」逻辑也支撑着<Link href="/knowledge/statistical-machine-learning">机器
            学习</Link>中的模型似然。懂得两个学派、以及各自何时合适，才是真正的技能。
          </p>
        </Callout>
      </KSection>
      ...
      <KSection id="refresher" eyebrow="10" title="60 秒回顾">
        <Callout type="refresher">
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li>
              一个<strong>可信区间</strong>意味着人们希望置信区间所表达的：P(参数在区间内 |
              数据) = 0.95。
            </li>
            <li>
