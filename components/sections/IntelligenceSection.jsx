...
const RIN_PROJ_PATH = "M 360,67 C 385,67 385,44 410,44 C 435,44 435,18 460,18";
...
const BENCH_PROJ_PATH = "M 360,155 C 385,155 385,147 410,147 C 435,147 435,137 460,137";
  ...
  branches: [
    {
      ...
      leaves: [
        ...
        },
        {
          ...
          proof:
            "CBS enforcement calendar derived from regression analysis of non-compliance patterns · CSIRO climate scenario planning: ENSO events → commodity price volatility → food-security risk.",
        },
      ],
    },
    {
      ...
      leaves: [
        {
          ...
          proof:
            "AutoRegressive time series with rolling windows (CSIRO): ENSO → commodity prices → conflict risk · CBS: regression, time-series decomposition, multivariate pattern detection on inspections data.",
        },
        {
    ...
    },
    {
      ...
      leaves: [
        ...
        },
        {
          ...
          proof:
            "UniMelb STEM Industry Mentor (Jul–Dec 2025) · UniMelb P2P Data Science Mentor (Aug–Sep 2024, Melbourne Plus People Leadership credential) · ANU CBE Analytics Mentor via Practera (Jul 2024, excellent student feedback). Teaching forces clarity of thought.",
        },
        {
      ...
      {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((v) => (
        <g key={v}>
          <line
            ...
            x2={470}
            y2={toY(v)}
            className="stroke-[#D0D0D0] dark:stroke-[#1C1C1C]"
            strokeWidth={0.5}
      ...
      {bands.map((b, i) => (
        <g key={i}>
          <rect
            ...
            className={
              i % 2 === 0
                ? "fill-[#E8E8E8] dark:fill-[#080808]"
                : "fill-[#F0F0F0] dark:fill-[#050505]"
            }
          />
          <text
            x={466}
            ...
            textAnchor="end"
            fontSize={8}
            className="fill-[#999] dark:fill-[#333]"
...
function BubblePanel() {
  ...
  // Trajectory lines
  const rinPath = RIN_BUBBLES.map(
    (b, i) => `${i === 0 ? "M" : "L"} ${toX(b.exp)},${toY(b.sen)}`
  ).join(" ");
  const benchPath = BENCHMARKS.map(
    (b, i) => `${i === 0 ? "M" : "L"} ${toX(b.exp)},${toY(b.sen)}`
  ).join(" ");
  ...
  return (
    <div>
      ...
      <p className="text-[11px] text-[#6E6E6E] dark:text-[#9A9A9A] font-mono mb-4 leading-relaxed">
        Plotted against the typical career trajectory for analytics professionals at each year of
        experience. Bubble size = cross-domain breadth (number of distinct technical domains
        actively used). Source: APS Career Pathfinder (APSC 2024) · LinkedIn Work Change Report
      ...
      <div className="relative overflow-x-auto">
        ...
        <svg viewBox="0 0 520 310" width="100%" style={{ overflow: "visible", minWidth: 320 }}>
          ...
          {[-2, -1, 0, 1, 2, 3, 4].map((v) => (
            <g key={v}>
              ...
              <text
                ...
                textAnchor="middle"
                fontSize={9}
                className={v < 0 ? "fill-[#888] dark:fill-[#444]" : "fill-[#999] dark:fill-[#555]"}
                fontFamily="monospace"
              >
                {v === -2 ? "Pre" : v < 0 ? "" : v === 0 ? "0" : `${v}`}
              </text>
          ...
          {[0, 2, 4, 6, 8, 10].map((v) => (
            <g key={v}>
              ...
              <text
                ...
                textAnchor="end"
                fontSize={8}
                className="fill-[#888] dark:fill-[#444]"
                fontFamily="monospace"
              >
                {v}
              </text>
          ...
          <text
            ...
            textAnchor="middle"
            fontSize={10}
            className="fill-[#999] dark:fill-[#555]"
            fontFamily="monospace"
          >
          ...
          {BENCHMARKS.map((b) => (
            ...
            >
              <circle
                ...
                fillOpacity={0.6}
                stroke={tone(b.borderColor, isDark)}
                strokeWidth={1}
                strokeDasharray="3,3"
              />
      ...
      {step >= 2 && (
        <div className="mt-6 space-y-3" style={{ animation: "fadeUp 0.4s ease 1.1s both" }}>
          ...
          <div className="border-l-2 border-[#CCC] dark:border-[#2A2A2A] pl-4">
            <p className="text-[10px] text-[#6E6E6E] dark:text-[#9A9A9A] uppercase tracking-widest mb-2 font-mono">
          ...
          <div className="border-l-2 border-[#CCC] dark:border-[#2A2A2A] pl-4">
            ...
            <p className="text-[10px] text-[#6E6E6E] dark:text-[#9A9A9A] mt-2 font-mono">
              Sources: SA Public Sector Wages Parity, VPS Agreement, NSW Crown Employees, APSC
              classification guides. Equivalence is indicative — actual duties and seniority vary by
              role and agency.
            </p>
          </div>
  ...
  return (
    <div>
      <p className="text-[11px] text-[#6E6E6E] dark:text-[#9A9A9A] font-mono mb-5 leading-relaxed">
        Compound growth index — not a capped score, but a ratio against the 2020 baseline. Strategic
        thinking and continuous improvement have no ceiling; every new context reactivates and
        deepens all prior knowledge. Benchmark: linear learner — single-sector, no cross-domain
        ...
        <svg viewBox="0 0 500 270" width="100%" style={{ overflow: "visible", minWidth: 320 }}>
          ...
          {yTicks.map((v) => (
            <line
              ...
              x2={460}
              y2={py(v)}
              className="stroke-[#DDD] dark:stroke-[#111]"
              strokeWidth={v === 1 ? 0.8 : 0.4}
              strokeDasharray={v === 1 ? "none" : "3,6"}
            />
          ))}
          ...
          <line
            ...
            x2={460}
            y2={240}
            className="stroke-[#CCC] dark:stroke-[#2A2A2A]"
            strokeWidth={1}
          />
          <line
            x1={60}
          ...
          {[2027, 2028].map((yr) => (
            <g key={yr}>
              <line
                ...
                x2={px(yr)}
                y2={244}
                className="stroke-[#DDD] dark:stroke-[#1A1A1A]"
                strokeWidth={1}
              />
              <text
                x={px(yr)}
                ...
                textAnchor="middle"
                fontSize={9}
                className="fill-[#CCC] dark:fill-[#2A2A2A]"
                fontFamily="monospace"
              >
                {yr}
              </text>
          ...
          {step >= 1 && (
            <>
              ...
              <path
                d={BENCH_PROJ_PATH}
                className="stroke-[#BBB] dark:stroke-[#1A1A1A]"
                strokeWidth={1}
                strokeDasharray="3,4"
                fill="none"
              ...
              <text
                ...
                y={py(2.64) - 6}
                fontSize={8}
                className="fill-[#999] dark:fill-[#2A2A2A]"
                fontFamily="monospace"
              >
                benchmark
              </text>
          ...
          {step >= 3 && (
            <g style={{ animation: "fadeUp 0.4s ease 0.2s both" }}>
              <line
                ...
                strokeOpacity={0.4}
              />
              <text x={px(2024) + 4} y={27} fontSize={8} fill="#FF3C3C" fontFamily="monospace">
                Breakout
              </text>
              <text x={px(2024) + 4} y={37} fontSize={8} fill="#FF3C3C" fontFamily="monospace">
                2024
              </text>
            </g>
          )}
          ...
          {step >= 3 && (
            <g style={{ animation: "fadeUp 0.4s ease 0.5s both" }}>
              <line
                ...
                x2={392}
                y2={py(2.64)}
                className="stroke-[#CCC] dark:stroke-[#686868]"
                strokeWidth={0.8}
              />
              <line
                x1={389}
              ...
              <text
                x={398}
                y={(py(5.37) + py(2.64)) / 2 + 13}
                fontSize={7}
                className="fill-[#999] dark:fill-[#444]"
                fontFamily="monospace"
              >
                gap
              </text>
      ...
      {step >= 3 && (
        ...
        >
          <div>
            ...
            <p className="text-2xl font-semibold tabular-nums text-black dark:text-white">
              5.37<span className="text-[#6E6E6E] dark:text-[#9A9A9A] text-base">×</span>
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-[#6E6E6E] dark:text-[#9A9A9A] mb-1 font-mono">
            ...
            <p className="text-2xl font-semibold tabular-nums text-[#FF3C3C]">
              32.7<span className="text-base">%</span>
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-[#6E6E6E] dark:text-[#9A9A9A] mb-1 font-mono">
            ...
            <p className="text-2xl font-semibold tabular-nums text-[#6E6E6E] dark:text-[#9A9A9A]">
              2.0<span className="text-base text-[#6E6E6E] dark:text-[#9A9A9A]">× ahead</span>
            </p>
          </div>
        </div>
      )}
      ...
      {step >= 3 && (
        <div className="mt-6 space-y-3" style={{ animation: "fadeUp 0.4s ease 0.8s both" }}>
          ...
          <div className="border-l-2 border-[#CCC] dark:border-[#2A2A2A] pl-4">
            <p className="text-[10px] text-[#6E6E6E] dark:text-[#9A9A9A] uppercase tracking-widest mb-2 font-mono">
              The two skills with no ceiling
            </p>
            <p className="text-sm text-[#6E6E6E] dark:text-[#9A9A9A] leading-relaxed font-light">
              Every technical domain has a practical depth boundary — there is only so much Python
              one person needs. Strategic thinking and continuous improvement do not. They grow
          ...
          <p className="text-[10px] text-[#AAA] dark:text-[#333] font-mono">
            CAGR = Compound Annual Growth Rate. Index calibrated against role deliverables and
            cross-domain breadth. Benchmark: linear progression modelled on IAPA Skills & Salary
            Survey (2023) median analyst trajectory.
          </p>
          <AiDeclaration />
        </div>
      )}
...
function FPBranch({ branch }) {
  ...
  return (
    <div>
      ...
      >
        <span
          className="flex-shrink-0 mt-1.5 w-[3px] h-5 rounded-full"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-black dark:text-white group-hover:text-[#555] dark:group-hover:text-[#DDDDDD] transition-colors flex items-center gap-2">
            {branch.label}
            <span className="text-[#6E6E6E] dark:text-[#9A9A9A] text-xs font-normal">
              {open ? "▾" : "▸"}
            </span>
          </p>
...
function FirstPrinciplesPanel() {
  ...
  return (
    <div>
      <p className="text-[11px] text-[#6E6E6E] dark:text-[#9A9A9A] font-mono mb-5 leading-relaxed">
        Each branch is a capability cluster. Each leaf is a real project, deliverable, or
        credentialled outcome. The fourth branch — Continuous Improvement — is the meta-skill that
        enables the other three to compound.
      </p>

      <button onClick={() => setRootOpen((o) => !o)} className="w-full text-left mb-6 group">
      ...
      {rootOpen && (
        <div
          className="ml-2 border-l border-[#DDD] dark:border-[#1E1E1E] pl-6 space-y-1"
        ...
        >
          ...
          <div className="mt-6 pt-4 border-t border-[#E0E0E0] dark:border-[#1A1A1A] space-y-2">
            <p className="text-[11px] text-[#6E6E6E] dark:text-[#9A9A9A] font-mono leading-relaxed">
              Strategy + Data Science + Engineering compound naturally when embedded in real
              problems. Continuous Improvement is the meta-layer that keeps the other three growing
              — and the one most frequently missing from a standard CV.
            </p>
            <AiDeclaration />
          </div>
        </div>
...
function AiDeclaration() {
  return (
    <p className="text-[9px] text-[#AAA] dark:text-[#333] font-mono mt-1 flex items-center gap-1.5">
      ...
      Analysis assisted by AI (Claude · Anthropic) · For reference only · Self-assessed
      approximations
    </p>
  );
}
    ...
    <section id="intelligence" className="py-24" aria-label="Intelligence Report">
      ...
      <div className="mt-4 border-t border-[#E0E0E0] dark:border-[#3D3D3D] pt-4">
        <p className="text-[10px] text-[#6E6E6E] dark:text-[#9A9A9A] leading-relaxed max-w-4xl">
          ...
          calibrated against real role deliverables. Industry benchmarks are derived from publicly
          available sources: IAPA Skills &amp; Salary Survey (2023), LinkedIn Work Change Report
          (2024), APSC Career Pathfinder and APS Workforce Data (2021–2024), and Randstad Gen Z
          Workplace Blueprint (2025). Individual trajectories vary significantly — this analysis
          reflects a particular career path, not a universal measure of performance. Generated with
          the assistance of an AI tool (Claude · Anthropic).
        </p>
      </div>
