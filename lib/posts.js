      ...
      (parent.children || (parent.children = [])).push(
        kw === "from"
          ...
          : (() => {
              ...
              return m.length > 1 ? { key: m[0], out: m.slice(1).join(" -> ") } : { key: null, out: rest };
            })()
      );
      continue;
    }
    if (kw === "loop") { loop = rest || "loops back to the start"; continue; }
    if (kw === "note") { note = rest; continue; }
...
async function parsePost(slug) {
  ...
  const processor = remark().use(remarkGfm).use(remarkFlowPlugin).use(remarkHtml, { sanitize: false });
