    ...
    <>
      ...
      <div className="min-h-screen bg-white dark:bg-[#0A0A0A] flex flex-col">
        <div className="max-w-[680px] mx-auto px-6 md:px-12 py-20 md:py-28 flex-1">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mb-3">{t("ds.confusionMatrix.heading")}</h1>
          <p className="text-sm text-[#7A7A7A] mb-10">
            {t("ds.confusionMatrix.subtitle")}
          </p>
            ...
            <table className="w-full text-sm font-mono">
              <thead>
                <tr className="bg-[#F5F5F5] dark:bg-[#141414]">
                  <th className="text-left p-3 font-semibold text-[#1A1A1A] dark:text-white">{t("ds.confusionMatrix.thPredicted")}</th>
                  <th className="text-left p-3 font-semibold text-[#1A1A1A] dark:text-white">{t("ds.confusionMatrix.thActual")}</th>
                  <th className="text-left p-3 font-semibold text-[#1A1A1A] dark:text-white">{t("ds.confusionMatrix.thMatch")}</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(({ who, reality }, i) => (
                  <tr key={i} className="border-t border-[#E0E0E0] dark:border-[#3D3D3D]">
                    ...
                    <td className="p-3">{CORRECT[i] ? <span className="text-[#3D3D3D] dark:text-[#AAAAAA]">✓</span> : <span className="text-[#FF3C3C]">✗</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
