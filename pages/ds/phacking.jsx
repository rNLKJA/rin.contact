...
export default function PhackingPage() {
  ...
  const rows = t("ds.phacking.rows") || [];

  return (
    <>
      <Head>
        <title>{t("ds.phacking.metaTitle")}</title>
        <meta name="description" content={t("ds.phacking.metaDescription")} />
        <meta name="robots" content="noindex" />
        <link rel="canonical" href="https://rin.contact/ds/phacking" />
      </Head>
      <SeoHead
        title={t("ds.phacking.metaTitle")}
        description={t("ds.phacking.metaDescription")}
        path="/ds/phacking"
        ogImage={{
          title: t("ds.phacking.ogTitle"),
          subtitle: t("ds.phacking.ogSubtitle"),
          section: "ds",
        }}
        locale={locale}
        noindex
      ...
      <div className="min-h-screen bg-white dark:bg-[#0A0A0A] flex flex-col">
        <div className="max-w-[680px] mx-auto px-6 md:px-12 py-20 md:py-28 flex-1">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mb-3">
            {t("ds.phacking.heading")}
          </h1>
          <p className="text-sm text-[#7A7A7A] mb-10">{t("ds.phacking.subtitle")}</p>

          <div className="border border-[#E0E0E0] dark:border-[#3D3D3D] p-6 font-mono text-xs space-y-4">
            {rows.map((row, i) => (
              <div key={i}>
                <p className="text-[#FF3C3C] mb-1">{row.label}</p>
                <p className="text-[#1A1A1A] dark:text-white">{row.text}</p>
              </div>
            ))}
          </div>

          <p className="text-[9px] text-[#AAAAAA] mt-6 font-mono">{t("ds.phacking.footnote")}</p>
