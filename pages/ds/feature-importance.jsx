    ...
    <>
      ...
      <div className="min-h-screen bg-white dark:bg-[#0A0A0A] flex flex-col">
        <div className="max-w-[680px] mx-auto px-6 md:px-12 py-20 md:py-28 flex-1">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mb-3">{t("ds.featureImportance.heading")}</h1>
          <p className="text-sm text-[#7A7A7A] mb-10">
            {t("ds.featureImportance.subtitle")}
          </p>

          <div className="space-y-4">
            {FEATURES.map(({ importance, bar }, i) => (
              <div key={i} className="flex items-center gap-4">
                <span className="font-mono text-xs text-[#1A1A1A] dark:text-white w-28 flex-shrink-0">{names[i]}</span>
