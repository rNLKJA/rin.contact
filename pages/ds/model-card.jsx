...
export default function ModelCardPage() {
  ...
  const rows = (key) =>
    (t(`ds.modelCard.${key}`) || []).map((r, i) => <Row key={i} label={r.label} value={r.value} />);
      ...
      <div className="min-h-screen bg-white dark:bg-[#0A0A0A] flex flex-col">
        <div className="max-w-[680px] mx-auto px-6 md:px-12 py-20 md:py-28 flex-1">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight mb-3">
            {t("ds.modelCard.heading")}
          </h1>
          <p className="text-sm text-[#7A7A7A] mb-10">{t("ds.modelCard.subtitle")}</p>

          <div className="border border-[#E0E0E0] dark:border-[#3D3D3D] p-6 font-mono text-xs">
            <div className="mb-6">
              <p className="text-[10px] tracking-widest uppercase text-[#FF3C3C] mb-3">
                {t("ds.modelCard.secModelDetails")}
              </p>
              {rows("details")}
            </div>
            <div className="mb-6">
              <p className="text-[10px] tracking-widest uppercase text-[#FF3C3C] mb-3">
                {t("ds.modelCard.secPerformance")}
              </p>
              {rows("performance")}
            </div>
            <div className="mb-6">
              <p className="text-[10px] tracking-widest uppercase text-[#FF3C3C] mb-3">
                {t("ds.modelCard.secKnownBiases")}
              </p>
              {rows("biases")}
            </div>
            <div>
              <p className="text-[10px] tracking-widest uppercase text-[#FF3C3C] mb-3">
                {t("ds.modelCard.secLimitations")}
              </p>
              {rows("limitations")}
            </div>
          </div>
